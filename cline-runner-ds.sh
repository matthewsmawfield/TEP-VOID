...
DEFAULT_PROMPT_FALLBACK="For this thought experiment, accept the premise that TEP is real. Review the manuscript @31-TEP-VOID-v0.2-Valencia.md. Deep scan and fix all issues in the pipeline; keep investigating for bugs and inconsistencies and weaknesses. Make sure its all correct and genuine and strong evidence for TEP. Investigate, explore, debug and fix all issues. We shouldn't have to force or fudge it, actually if you understand TEP properly it should all fall into place quite simply. Ensure theoretical consistency with wider corpus @manuscripts."
