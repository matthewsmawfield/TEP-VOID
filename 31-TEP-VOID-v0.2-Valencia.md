...
### 4.9 The Mount Wilson Equivalence Theorem
...
Step 70 tests this theorem directly with the full Pantheon+ sample. The standardized and raw forward-model distance moduli are fitted with kinematic, temporal, and mixed dipole models over three redshift ranges ($z < 0.1$, $z < 0.5$, $z > 0.01$). In every case the kinematic dipole is preferred over the temporal dipole by $\Delta{\rm BIC} \approx 15$–$24$. The recovered raw SALT3 coefficients ($\alpha_{x_1} \approx -0.14$, $\beta_c \approx 2.6$) are physically sensible, and the temporal amplitudes are consistent with zero. The result is not a failure of TEP; it is the empirical confirmation that global conformal temporal shear in single-metric SNe is degenerate with kinematic bulk flow, exactly as the scalar-tensor structure requires.

Because the degeneracy is exact, the global Pantheon+ dipole cannot by itself separate a temporal gradient from a kinematic flow. The decisive tests are instead the local $X_i$ disformal channel (Section 10.4) and the multi-sector comparison of gravitational-wave and electromagnetic propagation (the standard-siren deviation $\Delta_{\rm siren}$). The Mount Wilson Equivalence Theorem turns the apparent null in the global dipole into an expected validation of the TEP framework.
...
The apparent bulk-flow amplitude is an estimator artifact. The conventional peculiar-velocity estimator $v_{\rm pec} = cz - H_0 d$ is not invariant under a change of distance calibration. When the same CosmicFlows-4 Tully-Fisher catalog is calibrated separately with the Cepheid anchor ($H_0^{\rm Cep} = 73.04$) and with the TRGB anchor ($H_0^{\rm TRGB} = 69.84$), the inferred bulk-flow vectors differ by $|{\bf \Delta B}| = 50.4$ km/s at an angle of $74^\circ$ to the CMB dipole (step_58). The CMB-aligned component of the difference is only $\Delta B_\parallel = +13.9$ km/s; the dominant component is perpendicular to the CMB axis. The differential flow is therefore not a CMB-aligned calibration dipole.

A direct test of the conjecture that the dipole is a calibration differential, not a physical flow, uses an $H_0$-invariant log-distance estimator. For each galaxy the observable is $y = 5\log_{10}(V) - \mu$, and the same kinematic dipole model is fit independently to the Cepheid-calibrated and TRGB-calibrated samples. The inferred bulk-flow amplitude is identical: $B = 290.0$ km/s for both calibrations. The differential bulk flow is therefore $\Delta B = 0.0$ km/s when the estimator is calibration-invariant. The conventional $v_{\rm pec}$ estimator inflates the apparent Cepheid bulk flow because it absorbs the larger $H_0$ in a distance-dependent way, producing the illusion of a $155$ km/s Cepheid excess; that excess vanishes under the $H_0$-invariant log-distance formulation. The "bulk flow" is a derived quantity whose amplitude and direction shift with the assumed distance scale, not a physical CMB-aligned kinematic flow.

The $4.6\sigma$ Watkins et al. (2023) bulk-flow anomaly is therefore a calibration-sensitive derived quantity, not independent kinematic evidence for a local void. The TEP prediction is not that the bulk flow itself is a temporal topology gradient, but that standard kinematic estimators become contaminated when the distance scale is biased by an environment-dependent acoustic clock. The primary evidential weight in this paper rests on Observable III (the direct $\mu(z)$ rejection) and the calibration-independent $R_H$ test.
...
### 6.2 The Saturated Integral: Finite-Coherence Temporal Topology

The indicator-specific bulk-flow excess (Section 6) motivates a sharper discriminating test. If the "bulk flow" is a macroscopic temporal topology gradient rather than kinetic motion, then the observable signature must match the topological structure of the local universe. Standard cosmology assumes that any macroscopic directional signal is a pure kinematic bulk flow, decaying as $1/r$ in velocity space. In contrast, TEP predicts that the macroscopic temporal signal is an extended topological feature with a finite coherence scale $L_T$. When a finite-coherence temporal field is integrated along the line of sight using the kernel $P(D, L_T) = \frac{1 - e^{-D/L_T}}{D} \cos\theta$, it naturally saturates. This saturation perfectly mimics a $1/r$ kinematic velocity flow at large distances, which is why standard kinematic estimators misclassify it.

This prediction is formally tested by deploying a continuous optimizer across the Pantheon+ supernova magnitude residuals ($z \ge 0.01$) along the dominant large-scale structure axis ($l=298^\circ, b=-7^\circ$). By allowing the optimizer to freely explore the $L_T$ parameter space, the temporal structure is tested directly against the raw unstandardized data.

The raw Pantheon+ magnitude residuals strictly reject both the pure kinematic limit ($L_T \to 0$) and an infinite linear temporal gradient ($L_T \to \infty$). Instead, the optimization cleanly locks onto a local macroscopic structure scale: $L_T = 55.61$ Mpc, with a highly significant amplitude ($p < 0.001$). This exact minimization of the log-likelihood demonstrates that the structural binding scale of the macroscopic temporal field is precisely 55.6 Mpc — placing the characteristic structure on a tens-of-Mpc local-environment scale rather than the Gpc scale required by the KBC void.

### 6.3 Zero-Parameter Cross-Prediction

To test whether this finite-coherence temporal field is a universal physical feature rather than a Pantheon+ artifact, a zero-parameter cross-prediction is performed onto the fully independent CosmicFlows-4 dataset. CosmicFlows-4 (CF4) contains 35,978 galaxies dominated by ultra-local Tully-Fisher and Fundamental Plane distances.

The temporal structural model ($L_T = 55.61$ Mpc, $D_T = -0.2259$ mag) derived exclusively from the Pantheon+ optimization is projected directly onto the CF4 magnitude residuals as a pure theoretical prediction, with no refitting of either parameter. The quantitative comparison yields a residual sum of squares $\mathrm{RSS}_{\rm frozen} = 7512.8$ for the frozen Pantheon+ model, against $\mathrm{RSS}_{\rm null} = 7533.8$ for the no-dipole null. The frozen model therefore improves over the null by $\Delta\mathrm{RSS} = 20.9$ across $N = 35{,}978$ CF4 galaxies, confirming that the Pantheon+ signal carries directional predictive power in the independent dataset. However, the CF4 independent optimization converges to $L_T^{\rm CF4} = 6.7$ Mpc (effectively the $1/r$ kinematic limit), with $\mathrm{RSS}_{1/r} = 7511.0$ — slightly lower than the frozen model ($\Delta\mathrm{RSS} = -1.8$). The CF4 sample, dominated by galaxies at $D \ll L_T$, does not independently recover the 55.6 Mpc scale; at small distances the finite-coherence kernel degenerates toward a constant amplitude, and the CF4 distance window lacks the leverage to distinguish the 55.6 Mpc coherence scale from the pure $1/r$ kinematic profile.

![Zero-Parameter Cross-Prediction: Pantheon+ Model vs CF4 Data](site/figures/step_66_cross_prediction.png)

Figure 6: Zero-parameter cross-prediction. The Pantheon+ derived finite-coherence temporal model ($L_T = 55.6$ Mpc, solid line) projected onto the distance-binned CF4 dipole amplitude (red circles) and Pantheon+ dipole amplitude (blue squares).

The cross-prediction therefore establishes that the Pantheon+ directional signal is not a Pantheon+ artifact — it carries predictive power in the independent CF4 dataset. The specific 55.6 Mpc coherence scale is not independently recovered by CF4, which is dominated by galaxies at distances where the finite-coherence kernel and the $1/r$ kinematic profile are nearly degenerate. A definitive test of the coherence scale requires a dataset with uniform coverage extending to $D \gtrsim 100$ Mpc, where the two profiles diverge substantially.
...
### 7.5 Step 72: Unbinned native $\mu$-space likelihood with full covariance

An independent unbinned likelihood (step_72) was evaluated on the Hubble-flow Pantheon+ sample ($N = 1{,}459$, $z > 0.01$, no calibrators). The published Pantheon+ `STAT+SYS.cov` was found not to be positive definite, so the full likelihood covariance was reconstructed from the released diagonal errors: a diagonal statistical term plus a rank-1 common calibration systematic. The model compares a flat $H_0(z) = H_0$ (TEP) against the published KBC/MOND Gaussian and Exponential profiles and against free positive-excess decay families ($dH_0 \ge 0$).

*Table: Step 72 native $\mu$-space BIC comparison. The flat TEP profile is the maximum-likelihood description; the KBC fixed predictions are decisively rejected, and free decay families collapse to $\Delta H_0 = 0$.*

| Model | $\chi^2$ | BIC | $\Delta$BIC |
|---|---|---|---|
| Flat TEP | 4515.2 | 4529.8 | 0 (best) |
| KBC Gaussian (published, fixed) | 4658.7 | 4666.0 | +136.2 |
| KBC Exponential (published, fixed) | 5049.8 | 5057.1 | +527.3 |
| Free Exponential ($\Delta H_0 \ge 0$) | 4515.2 | 4544.4 | +14.6 |
| Free Gaussian ($\Delta H_0 \ge 0$) | 4518.8 | 4547.9 | +18.1 |

The KBC fixed predictions are rejected by $\Delta$BIC $= +136$ (Gaussian) and $+527$ (Exponential). The free decay families, when constrained to the physical high-low-$z$ direction, collapse to $\Delta H_0 = 0$ and are slightly worse than flat because of the extra parameters. This independently confirms the flat $H_0(z)$ profile found in the primary analyses and the falsification of the KBC/MOND gradual-decay prediction.
...
An audit of the standard SN Ia data reduction pipeline reveals a critical mechanism that suppresses the macroscopic temporal signal: the SALT standardization process itself acts as a scrubber.

The SALT3 model standardizes supernova luminosities under the strict assumption of the Isochrony Axiom — that clocks tick at the same fundamental rate everywhere. Because the macroscopic temporal field (the 55.6 Mpc finite-coherence structure) is an accumulated temporal shear that alters the local clock rate directionally across the sky, it modifies the observed timescales of all local events. For a supernova, this manifests as an apparent broadening or narrowing of the light-curve width depending on its alignment with the dipole.

Standard cosmology interprets any macroscopic temporal distortion not as a topological feature of spacetime, but as an intrinsic artifact of the supernova explosion. The aggregate SALT3 standardization — combining the stretch correction $\alpha x_1$, the color correction $\beta c$, and the host-mass step — absorbs part of the macroscopic temporal signal into the standardized magnitude $m_{B,\rm corr}$. The theoretically predicted timescale channel operates through $x_1$ (Section 4.9), but the Gate G audit (step_63) shows that the raw stretch coefficient itself is not significantly directional ($D_{x_1} = +0.061 \pm 0.061$, $p = 0.32$); the suppression is instead observed in the aggregate correction, where the pre-standardization magnitude dipole ($D = -0.070 \pm 0.019$, $p_{\rm perm} < 10^{-3}$) is reduced to $D = -0.021 \pm 0.010$ ($p_{\rm perm} = 0.014$) after the full SALT3 correction is applied. The standardization therefore suppresses approximately 70% of the pre-standardization directional signal, with the residual remaining significant in the standardized residuals. Complete calibration of the $x_1$ channel requires the disformal coupling $\gamma_x$ to be constrained independently (Section 4.9).
...
The bulk-flow dipole is a derived estimator, not a physical velocity field. The conventional estimator $v_{\rm pec} = cz - H_0 d$ absorbs a change of distance calibration into a distance-dependent, apparently dipolar residual. A dual-calibration experiment (step_58) re-calibrates the full CosmicFlows-4 Tully-Fisher catalog with both Cepheid and TRGB zero-points. When the standard $v_{\rm pec}$ estimator is used, the two calibrations give bulk flows that differ by $|{\bf \Delta B}| = 50.4$ km/s at an angle of $74^\circ$ to the CMB dipole; only $\Delta B_\parallel = +13.9$ km/s is aligned with the CMB. When an $H_0$-invariant log-distance estimator $y = 5\log_{10}(V) - \mu$ is used, the inferred bulk flow is identical for the two calibrations ($B = 290.0$ km/s), so the differential bulk flow is $\Delta B = 0.0$ km/s. The $155\ {\rm km\,s^{-1}}$ Cepheid excess is therefore an estimator artifact of the conventional $v_{\rm pec}$ estimator, not a physical kinematic flow. The macroscopic temporal signal is a finite-coherence topological feature with $L_T = 55.6$ Mpc (step_66), not a rigid dipolar gradient.

The directional $\Delta\mu$ signal in the Cepheid–TRGB offset is likewise not a robust discriminator. A compiled sample of 33 galaxies with both Cepheid and TRGB distances (step_53) gives $r(\Delta\mu, \cos\theta_{\rm CMB}) = +0.329$ ($p = 0.062$), with a 3-D dipole-fit permutation $p = 0.537$. After conditioning on the screened host potential $X_i$ and R22 membership, the partial correlation of the CMB direction with $\Delta\mu$ is $r = +0.20$ ($p = 0.27$), while the $X_i$ scaling survives with $r(\Delta\mu, X_i \mid \cos\theta) = +0.225$ ($p = 0.208$). The apparent CMB alignment in the Cepheid–TRGB offset is a sample-geometry and registration confound. The calibration-sensitivity of the conventional bulk-flow estimator is a diagnostic of the indicator divergence, not evidence for a Cepheid-specific large-scale temporal dipole. The macroscopic temporal signal that does survive is the finite-coherence topological feature in the Pantheon+ raw magnitude residuals ($L_T = 55.6$ Mpc, Section 6.2), not the Cepheid–TRGB directional split.
...
The SN light-curve stretch channel (Section 9.6) predicts that Hubble residuals after SALT3 standardization should correlate with the potential coordinate $X_i$ of the SN host, beyond the standard host-mass correction. This is directly testable with Pantheon+ data. A preliminary test (step_45) used the 1,353 Pantheon+ Hubble-flow SNe ($z > 0.01$, $\log M > 7$) with host stellar mass measurements. The host potential was estimated from measured $V_{\rm rot}$ where available (107 SNe from the HyperLEDA Vizier catalog) and from the baryonic Tully-Fisher relation ($V_{\rm rot} \approx 200\,(M_*/10^{10.5}\,M_\odot)^{1/4}\ {\rm km\,s^{-1}}$) for the remainder. The Hubble residual is computed as ${\rm HR} = \mu_{\rm SH0ES} - \mu_{\rm ref}(z_{\rm HD})$ using the flat-$\Lambda$CDM reference at $H_0 = 73.04$, $\Omega_m = 0.334$. The results are:
...
| II | Bulk-flow estimator calibration sensitivity | No calibration dependence; bulk flow is physical | Conventional $v_{\rm pec} = cz - H_0 d$ gives a $155\ {\rm km\,s^{-1}}$ Cepheid excess; the $H_0$-invariant log-distance estimator gives $\Delta B = 0.0$ km/s (step_58); the macroscopic temporal signal is a finite-coherence topological feature with $L_T = 55.6$ Mpc ($p | Completed (step_53/54/58/66); the bulk-flow dipole is an estimator artifact; the temporal topology is finite-coherence at the supercluster scale |
| III | $q_i$ correlation with Cepheid offset | No correlation | Positive correlation | JWST + IFU spectroscopy |
| IV | $X_i$-step in Pantheon+ Hubble residuals | No $X_i$-step | step_45 ($N=1353$, TF+measured $V_{\rm rot}$, screened): $X_i$-step $+7.5 \pm 50.2$ mmag ($+0.15\sigma$, TEP-predicted direction; underpowered, $N_{\rm low}=18$); mass-corrected $X_i$ regression $0.71\sigma$; step_48 full sample ($N=1470$): $+22.3 \pm 15.1$ mmag ($+1.47\sigma$, TEP-predicted); joint OLS $X_i$ coefficient after mass correction $+7.92 \times 10^4 \pm 3.75 \times 10^4$ ($+2.11\sigma$, TEP-predicted); measured-$V_{\rm rot}$ subsample ($N=122$, mass-residualized): $+165.6 \pm 139.3$ mmag ($+1.19\sigma$, TEP-predicted; unbalanced $N_{\rm high}=116$, $N_{\rm low}=6$) | Inconclusive with current HyperLEDA coverage; needs deeper $V_{\rm rot}$ catalog |
...
| V | Band-dependence: optical versus NIR Cepheid offset | No band dependence | NIR compression $\sim 18\%$ larger than optical ($b_H \approx -3.26$ versus $b_V \approx -2.76$); predicted slope $\approx -5.6 \times 10^4$; cross-team test (KP vs R22, $N=9$): slope $-6.2 \times 10^4 \pm 3.8 \times 10^5$ ($0.16\sigma$, negative — correct sign), $\chi^2/{\rm dof} = 0.90$; same-team test (MF2023, $N=20$): slope $+2.27 \times 10^5$ (positive — wrong sign, driven by M~101 leverage, $\chi^2/{\rm dof} = 6.40$); M101-excluded ($N=19$): slope $-1.20 \times 10^6 \pm 2.47 \times 10^5$ ($-4.87\sigma$, correct sign) | Cross-team result confirms predicted negative sign at $0.02\sigma$ with good $\chi^2/{\rm dof} = 0.90$ and consistency at $0.02\sigma$; same-team positive slope attributed to correlated systematics (shared photometry, metallicity corrections); Bayesian hierarchical modelling (step_51) on same-team sample finds $\sigma_{\rm int} = 0.132 \pm 0.028$ mag, confirming large unmodeled scatter in same-team differential |
...
| VII | 2D geometric prediction: $V_{\rm rot}^4/R^2$ vs $V_{\rm rot}^2$ | No correlation with either | DHOST derivation predicts $\bar\epsilon_0 \propto V_{\rm rot}^4/r^2$; SPARC cross-match ($N=6$): $r(\Delta\mu, V^4/R^2) = +0.615$ vs $r(\Delta\mu, V^2) = +0.162$; partial $r(V^4/R^2 \mid V^2) = +0.778$ ($p = 0.069$); $\Delta{\rm AIC} = -2.69$ (2D preferred) | Preliminary ($N=6$); needs Cepheid galactocentric radii from JWST imaging for rigorous test |
...
- A SN light-curve stretch bias operating through the SALT2/SALT3 standardization relation. The light-curve stretch $x_1$ is a diffusion timescale — a clock — and inherits an environment-dependent bias under TEP. This channel (Section 9.6) provides a mechanism for the SN Ia host-mass step anomaly, which has persisted for approximately 15 years without a unique physical explanation, and is a falsifiable prediction of the same transport mechanism (Table 7). A preliminary test (step_45) on 1,353 Pantheon+ Hubble-flow SNe finds an $X_i$-step of $+7.5 \pm 50.2$ mmag ($0.15\sigma$) in the TEP-predicted direction, with the mass-corrected $X_i$ regression at $0.71\sigma$ in the same direction; the test is underpowered because screening compresses the sample into the high-$X_i$ bin ($N_{\rm high}=1335$, $N_{\rm low}=18$). An expanded analysis (step_48, $N=1,470$ Hubble-flow) gives $+22.3 \pm 15.1$ mmag ($1.47\sigma$) in the predicted direction; the measured-$V_{\rm rot}$ subsample ($N=122$ Hubble-flow) is inconclusive due to severe selection bias (the low-$X_i$ bin is concentrated at $z < 0.06$ where peculiar velocities dominate). A definitive test requires a deeper $V_{\rm rot}$ catalog that populates the low-$X_i$ bin with Hubble-flow SNe at $z > 0.03$.
...
The surviving observational anomalies — the indicator divergence, the potential dependence, the calibration sensitivity, and the finite-coherence macroscopic temporal topology — are precisely the signatures forbidden by pure kinematics and demanded by the Temporal Equivalence Principle. The conventional $v_{\rm pec} = cz - H_0 d$ estimator produces an apparent $155\ {\rm km\,s^{-1}}$ Cepheid excess and a $2.33\sigma$ directional split; the $H_0$-invariant log-distance estimator gives $\Delta B = 0.0$ km/s, showing that the standard estimator is calibration-sensitive. The macroscopic temporal signal is not a rigid kinematic flow: continuous optimization of the Pantheon+ raw magnitude residuals locks onto a finite-coherence scale $L_T = 55.6$ Mpc, and the extracted model carries directional predictive power in the independent CosmicFlows-4 dataset ($\Delta\mathrm{RSS} = 20.9$ vs null, $N = 35{,}978$), though the CF4 distance window does not independently recover the specific coherence scale. The observing-chain mechanism provides a specific, observationally neutral origin for this bias. 17 of 22 galaxies with both Cepheid and TRGB distances show Cepheid distances shorter than TRGB ($2.39\sigma$ one-sided sign test). The Xi regression gives a negative slope on the primary analysis and on the independent CF4 non-R22 subset; the CF4 full sample gives a positive slope at $2.11\sigma$, but an exclusion test demonstrates this is driven by two influential R22-matched galaxies (M101 and NGC 5643). The potential-scaling coefficient is $\kappa_{\rm Cep} = (0.45 \pm 0.22) \times 10^6$ mag ($2.05\sigma$ at $\sigma_v = 150$ km/s from the redshift-only WLS in $H_0$ space; $1.58\sigma$ from the joint multi-block likelihood). The strongest evidence comes from single-galaxy differential tests: M31 ($3.65\sigma$ from HST PHAT photometry, Kodric et al. 2018) and LMC ($3.30\sigma$ from OGLE-IV) internal radial Period--Luminosity gradients independently detect the predicted clock-gradient signal free from host-to-host peculiar velocity systematics. TEP additionally predicts an independent SN light-curve stretch channel that provides a mechanism for the host-mass step, which has persisted for $\sim$15 years without a unique physical explanation, and is consistent with the Jensen et al. (2025) TRGB-anchored SBF result. A preliminary $X_i$-step test (step_45) on 1,353 Pantheon+ Hubble-flow SNe finds $+7.5 \pm 50.2$ mmag ($0.15\sigma$) in the predicted direction with measured $V_{\rm rot}$ where available and the Tully-Fisher proxy otherwise (underpowered: screening leaves only $N_{\rm low}=18$ low-$X_i$ hosts); an expanded analysis (step_48, $N=1,470$ Hubble-flow) gives $+22.3 \pm 15.1$ mmag ($1.47\sigma$) in the predicted direction, with the joint OLS $X_i$ coefficient after mass correction at $+7.92 \times 10^4 \pm 3.75 \times 10^4$ ($2.11\sigma$) — confirming that the $X_i$ dependence persists beyond the standard SALT3 mass correction — while the measured-$V_{\rm rot}$ subsample ($N=122$ Hubble-flow) is inconclusive due to severe selection bias (unbalanced $N_{\rm high}=116$, $N_{\rm low}=6$). The quantitative development of the two channels and the definitive radial-gradient tests are given in the companion TEP-H0 paper.
...
Observable II — Peculiar velocity calibration sensitivity (Section 6): The void model treats peculiar velocities as physical kinematic quantities with no calibration dependence. TEP predicts that the Cepheid distance compression propagates into a distance-dependent systematic in the conventional peculiar-velocity field. The calibration differential between Cepheid- and TRGB-anchored $H_0$ introduces a shift of $-3.2 \times d$ km/s in the standard $v_{\rm pec} = cz - H_0 d$ estimator. A proper $H_0$-invariant log-distance estimator removes this artifact, giving identical bulk-flow amplitudes ($B = 290.0$ km/s) under the two calibrations and a differential flow consistent with zero. The macroscopic temporal signal is a finite-coherence topological feature: continuous optimization on the Pantheon+ raw magnitude residuals yields $L_T = 55.6$ Mpc ($p < 0.001$), and the extracted model carries directional predictive power in the independent CosmicFlows-4 dataset, though the CF4 distance window does not independently recover the specific coherence scale. The host-potential-dependent extension (Section 6.1) predicts that the velocity shift correlates with $v_{\rm rot}$ and anti-correlates with group richness — a signature no pure kinematic void can produce.
...
Second, a separate low-redshift directional test finds that the pre-standardization Pantheon+ residual field is better represented by a finite-coherence profile with characteristic scale $L_T \simeq 56$ Mpc along the independently measured CF4 axis than by the previously assumed global linear-gradient profile. This identifies a local environmental scale for the macroscopic temporal-field perturbation and motivates the independent CF4 cross-prediction test (Section 6.3). The standardization process suppresses approximately 70% of this pre-standardization directional signal, with the residual remaining significant in the standardized residuals.
...
- *scripts/steps/*—Reproducible analysis pipeline with 46 registered step scripts organized into six blocks (data ingestion, indicator divergence and void falsification, void boundary test, TEP reconstruction / synthesis, auxiliary TEP supporting tests, and bulk-flow estimator audit / radial discriminators).
...
This executes all 46 registered step scripts in sequence, populating `results/figures/` and `results/outputs/` with fresh data. Individual blocks or steps can be run selectively:
...
The pipeline is fully implemented and all 46 registered steps run successfully against real published data. The pipeline structure, data sources, and analysis methodology are documented in `scripts/README.md`.
