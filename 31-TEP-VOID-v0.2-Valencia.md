...
The non-radial $g_{0i}$ structure induced by orbital motion and stellar pulsation eigenvalue modification is represented by an $\mathcal{O}(1)$ transport efficiency coefficient $\eta_P$ derived from modified stellar structure and the linear adiabatic wave equation (step 52):
...
| II | Bulk-flow estimator calibration sensitivity | No calibration dependence; bulk flow is physical | Conventional $v_{\rm pec} = cz - H_0 d$ gives a $155\ {\rm km\,s^{-1}}$ Cepheid excess; the $H_0$-invariant log-distance estimator gives $\Delta B = 0.0$ km/s (step_58); the macroscopic temporal signal is a finite-coherence topological feature with $L_T = 55.6$ Mpc ($p \lt 0.001$, step_66), carrying directional predictive power in the independent CF4 dataset | Completed (step_53/54/58/66); the bulk-flow dipole is an estimator artifact; the temporal topology is finite-coherence at the supercluster scale |
