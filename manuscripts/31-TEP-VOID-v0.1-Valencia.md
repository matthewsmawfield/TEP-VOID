...
### 1.1 Constraints on Early-Time Solutions

Early-time solutions are tightly constrained by the CMB acoustic structure, BAO, BBN, growth and late-time age consistency. These constraints substantially restrict modifications capable of reducing the sound horizon while retaining the observed cosmological fit, motivating direct tests of late-time and calibration alternatives.

### 1.2 Three Competing Late-Universe Frameworks

Three major research frameworks compete to resolve the tension through late-universe modifications:
...
This result is critical to the present analysis. The JWST validation establishes that the local $H_0$ measurement is an authentic physical measurement: the telescopes are measuring the exact light the stars are emitting with negligible instrumental error. The question is not whether the photometry is correct—it is—but whether the standard physical interpretation of that light (the universal Leavitt Law) is complete.
...
Watkins et al. (2023) analyzed the CosmicFlows-4 catalog, providing highly accurate measurements of the bulk flow of galaxies on scales of $100$--$250\ h^{-1}\ {\rm Mpc}$. The ΛCDM model strictly predicts that as the survey volume increases, the bulk flow velocity should rapidly converge toward zero, as the universe becomes highly homogeneous on scales exceeding 100 Mpc. However, the CosmicFlows-4 data reveals a monotonically rising bulk-flow curve. Galaxies within this radius exhibit bulk flows significantly faster than theoretically permissible, resulting in a severe $4.8\sigma$ tension with standard cosmology at a scale of $200\ h^{-1}\ {\rm Mpc}$.
...
| Bulk flow ($200\ h^{-1}$ Mpc) | Rising velocity curve | Rapid convergence to 0 km/s | $4.8\sigma$ |
...
|---|---|---|---|
| Distance-modulus channel coefficient | $\kappa_{\rm Cep}$ | $(0.452 \pm 0.220) \times 10^6$ mag | Fitted directly from $H_0$-space regression (Section 6) |
...
The scalar field traces the Newtonian potential, scaled by $2\beta_A M_{\rm Pl}$ to give $[\delta\phi] = M$. The full dynamical time field is therefore $\phi(t, r) = \phi_0(t) + 2\beta_A\,M_{\rm Pl}\,\Phi_N(r)$ (where we write the normalized time $\tau = t/t_0$ as $t+\psi$ in the metric expansion below).
...
The potential coordinate is $X_i = (V_{\rm rot}^2/2 - U_{\rm ref})/c^2 \approx V_{\rm rot}^2/(2c^2)$. We define the effective empirical normalization $\bar\epsilon_0$ via $\epsilon_{\rm env} = \bar\epsilon_0\,X_i$, which relates the phenomenological amplitude to the underlying action parameters:
...
The transport efficiency factor $\mathcal{T}_i$ is a parameterization of the synchronization holonomy. For a photon traveling radially ($d\tilde{\tau}^2 = 0$, $d\Omega = 0$), the time of flight is given by the root of the metric quadratic:
...
A rigorous derivation of this drift requires modeling the non-radial $g_{0i}$ structure induced by orbital motion, which we defer to future work. We therefore parameterize the synchronization holonomy transport factor with an $\mathcal{O}(1)$ transport efficiency coefficient $\eta_P$:

\begin{equation}
\mathcal{T}_i = 1 - \eta_P\,\epsilon_{\rm env} .
\end{equation}

This formally establishes the parameterized origin of the bias:
...
The disformal transport mechanism makes a falsifiable band-dependence prediction. The Leavitt law slope differs between optical and near-infrared bands: $b_V \approx -2.76$ versus $b_H \approx -3.26$. Since $\kappa_{\rm Cep} = |b|\,\kappa_P$, the TEP correction is $\sim 18\%$ larger in the NIR than in the optical. The predicted optical-to-NIR scaling provides a specific discriminant against conventional chromatic systematics once extinction, metallicity and reduction effects are controlled in a matched-host analysis. The CF4 Cepheid sample splits naturally into Key Project optical reductions and SH0ES NIR reductions, providing a natural test: the Cepheid-TRGB offset should be systematically larger in NIR-calibrated hosts than in optically-calibrated hosts at the same potential depth. This prediction is specific to the disformal transport channel and provides a discriminant against conventional chromatic systematics in a controlled matched-host analysis.
...
Applying this environmental clock correction to the Cepheid calibrators accounts for the $\approx 0.045$ mag per-host offset (at typical $X_i \sim 10^{-7}$ with $\kappa_{\rm Cep} = 0.452 \times 10^6$), with the maximum per-host correction reaching $\sim 0.09$ mag at the highest-$X_i$ host (M101). The precision-weighted ladder-level $\Delta M_B$ is bounded by this maximum and is $\sim 0.025 \pm 0.021$ mag for the calibrator average. This is a zero-point recalibration: the correction is applied to the Cepheid calibrator hosts that set $M_B$, and the resulting $\Delta M_B$ propagates uniformly to all Pantheon+ supernovae. The simple calibrator-average shift is $+0.025 \pm 0.021$ mag. Its exact distance-ladder propagation depends on the estimator: the public SH0ES design matrix gives $H_0 = 71.77 \pm 0.99\ {\rm km\,s^{-1}\,Mpc^{-1}}$, while the unified host-level reconstruction of TEP-H0 gives $H_0 = 66.65 \pm 1.58\ {\rm km\,s^{-1}\,Mpc^{-1}}$, consistent with the CMB value at $0.45\sigma$. Section 9.5 distinguishes these two routes explicitly. The detailed quantitative analysis of this correction across the 37-host SH0ES sample is developed in the companion paper TEP-H0 (Paper 11).
...
The full 22-galaxy sample with $\Delta\mu$, $X_i$, screening factor $S_{\rm total}$, $v_{\rm rot}$, pipeline flag, and sign is released as a machine-readable CSV at `results/outputs/step_36_22galaxy_table.csv`.
...
*Variant B: CF4 registered 22* ($N=22$): $\Delta\mu = -0.080 \pm 0.024$ mag ($3.30\sigma$ unweighted; $-0.105 \pm 0.017$ weighted, $6.13\sigma$). All 22 CF4 galaxies with both measurements. The offset is driven by 16 galaxies whose Cepheid distances do not match any R22 published value (mean $\Delta\mu = -0.110$ for non-R22 galaxies versus $-0.028$ for R22-matched).
...
| TEP-H0 raw | 18 | unscreened | $-1.17 \times 10^5 \pm 2.87 \times 10^5$ | $0.41$ | negative |
| CF4 non-R22 | 16 | screened | $-5.46 \times 10^5 \pm 5.60 \times 10^5$ | $0.98$ | negative |
| CF4 non-R22 | 16 | unscreened | $+7.29 \times 10^4 \pm 2.42 \times 10^5$ | $0.30$ | positive |
| CF4 full | 22 | screened | $+5.36 \times 10^5 \pm 2.54 \times 10^5$ | $2.11$ | positive |
| CF4 full | 22 | unscreened | $+4.03 \times 10^5 \pm 1.66 \times 10^5$ | $2.42$ | positive |

The table shows a mixed picture. The TEP-H0 raw data (rows 1--2) gives a negative slope in both screened and unscreened variants, consistent with the TEP prediction, but at low significance ($0.27\sigma$, $0.41\sigma$). The CF4 non-R22 subset (row 3) gives a negative slope at $0.98\sigma$ with screening, but a positive slope without screening (row 4). The CF4 full sample (rows 5--6) — the largest and most complete sample — gives a positive slope at $2.11\sigma$--$2.42\sigma$, opposite to the TEP prediction.
...
## 6. Consequence II: Peculiar Velocity Calibration Sensitivity

This section presents a deterministic consequence and consistency check, not an independent high-significance test. It illustrates how the indicator-specific distance divergence established in Section 5 propagates into the peculiar velocity field. Peculiar velocities are not directly observed; they are derived quantities, computed as $v_{\rm pec} = cz - H_0 d$, where $d$ is the distance inferred from a standard candle and $H_0$ is the assumed background expansion rate used to subtract the Hubble flow. If the distance calibration is biased, the inferred peculiar velocity field is systematically distorted.

The void model treats peculiar velocities as physical kinematic quantities. Under this framework, the $4.8\sigma$ bulk-flow anomaly in CosmicFlows-4 is independent evidence for the KBC void: galaxies are physically moving outward, and this motion is captured in the velocity field regardless of which distance indicator is used.
...
This shift is deterministic by construction (it is not a statistical test), but it quantifies the physical link between the Cepheid calibration bias and the bulk-flow anomaly. Using the full 38,053-entry CosmicFlows-4 group catalog, the shift reaches $-640\ {\rm km\,s^{-1}}$ at 200 Mpc and $-800\ {\rm km\,s^{-1}}$ at 250 Mpc. The RMS shift across the catalog is $839\ {\rm km\,s^{-1}}$, and 89% of galaxies have calibration-induced shifts exceeding $200\ {\rm km\,s^{-1}}$ — comparable to the typical peculiar velocity dispersion.

The bulk-flow amplitude is correspondingly sensitive: the mean reduction in the TRGB-calibrated bulk flow relative to the Cepheid-calibrated value is 3.3% across radial bins from 50 to 250 Mpc. Under the void model, the bulk flow is gravitational and should not depend on the $H_0$ calibration. Under TEP, the 3.3% reduction is the expected consequence of the indicator divergence (Section 5): the Cepheid-calibrated $H_0$ inflates the apparent bulk flow, and using the TRGB calibration partially removes this inflation.

The $4.8\sigma$ Watkins et al. (2023) bulk-flow anomaly is therefore not purely kinematic evidence for a local void. The velocity catalog is calibration-sensitive: switching from Cepheid-anchored to TRGB-anchored $H_0$ changes the measured bulk-flow amplitude by $3.3\%$. This directly bounds the level of distance-scale leakage into the bulk-flow statistic. The void model does not naturally predict this calibration dependence; TEP provides a natural consequence of the acoustic clock bias. However, Mazurenko et al. (2024) note that the properly estimated bulk-flow vector, unlike an individual peculiar velocity, is largely insensitive to the assumed $H_0$. The $3.3\%$ change reported here is computed from a vector average of recalculated peculiar velocities, not from the full Watkins/Mazurenko bulk-flow estimator. This result should therefore be interpreted as a prediction of calibration sensitivity, not as standalone observed evidence for TEP. The primary evidential weight in this paper rests on Observable III (the direct $\mu(z)$ rejection) and the calibration-independent $R_H$ test.

### 6.1 Host-Potential-Dependent Velocity Shift: A Calibration-Sensitivity Diagnostic
...
where $X_i = (S_{\rm total}\,u_\phi^2 - U_{\rm ref})/c^2$ is the screened TEP potential coordinate (Section 5.5). Since $\Delta v_i$ is constructed proportionally to $X_i$, a correlation between $\Delta v_i$ and any variable entering $X_i$ is partly built in by design. This is a prediction of the TEP framework, not independent observed evidence. Confirming it requires correlating actual velocity residuals — not TEP-constructed shifts — with $X_i$. The qualitative distinction from a kinematic void (which produces a distance-dependent shift independent of host mass) remains valid as a prediction.

Using the matched calibrator sample with measured potentials, the host-potential-dependent shift is of order a few tens of km/s (e.g., approximately $-54$ to $+16$ km/s for the 18 galaxies with potential data), not hundreds or thousands of km/s. Because $\Delta v_i$ is constructed proportionally to $X_i$, any correlation with variables that enter $X_i$ is partly built in by design. A kinematic void model predicts no such host-mass dependence, so a future correlation of actual velocity residuals with $X_i$ would still distinguish the two frameworks; the present estimate is a calibration-sensitivity diagnostic, not a discriminating observation. A rigorous test requires a larger sample with homogeneous Cepheid reductions and TRGB cross-checks, the subject of ongoing work.
...
The results isolate the origin of the declining trend. With SN-only data (configuration A), the MCMC fit yields a change of $-3.68$ km/s/Mpc from low-$z$ to high-$z$, but with a significance of only $1.93\sigma$ — not statistically significant. The high-redshift bins have large uncertainties ($\sigma_{H_0} \sim 7$ km/s/Mpc at $z > 0.7$) because Pantheon+ contains few SNe at those redshifts. This is consistent with Jia et al.'s own statement that the SN-only case has "poor constraints" at high redshift and that they "decided to add the observed Hubble parameter data and BAO data" to obtain tighter constraints.

With SN + $H(z)$ data (configuration B), the MCMC fit yields a change of $-5.92$ km/s/Mpc at $3.40\sigma$ (correlated) or $-5.46$ km/s/Mpc at $2.79\sigma$ after PCA decorrelation. The decline is present, but its significance ($2.79\sigma$ decorrelated) is lower than Jia et al.'s reported $5.6\sigma$, likely due to differences in binning (8 versus 10 bins), MCMC chain length, and the exact $H(z)$ dataset. The qualitative conclusion is the same: the declining $H_0(z)$ is driven by the $H(z)$ cosmic chronometer data, not by the Pantheon+ SN distance moduli alone.

The origin of the Jia et al. decline is therefore identified: it arises from the $H(z)$ and BAO data combined with the SN data, not from the Pantheon+ SN distance moduli alone. The SN-only configuration, using the full covariance and the correct piecewise $H_{\rm th}$ estimator, does not produce a significant decline ($1.93\sigma$). The native $\mu$-space likelihood likewise rejects the KBC gradual decay curves. An independent Pantheon+ tomographic analysis (Wang 2023, Eur. Phys. J. C 83, 813) found no obvious evidence for evolution of $H_0$ or $\Omega_m$ at the $2\sigma$ level using both equal-redshift and equal-count binning, consistent with the flat profile reported here.
...
Consistent with the methodological scope stated at the opening of Section 7, Pantheon+'s global $M_B$ calibration tests the spatial persistence of the tension, not its host-mass dependence. A computational proof is provided by step_35 in the TEP-VOID pipeline, which releases the global $M_B$ constraint and fits $M_B$ separately for massive and low-mass hosts. The residual mass step is only $0.013$ mag after the existing SALT2 mass correction and global-$M_B$ calibration, demonstrating that little host-mass structure remains in the released Pantheon+ moduli. This is consistent with masking of the TEP-predicted host dependence ($\sim 0.13$ mag); recovering that dependence requires the per-host Cepheid recalibration performed in TEP-H0 (Paper 11).

## 9. TEP Correction: Two-Channel Calibration Bias

Sections 5 through 8 have established that the published KBC/MOND gradual decay curves are rejected by the observed flat $H_0(z)$ profile. This section quantifies the physical scale of the TEP calibration bias by tracing the correction through two distinct channels: the Cepheid period-transport channel (calibrator hosts $\to$ $M_B$ zero-point) and the SN light-curve stretch channel (SN standardization $\to$ Hubble-flow residuals). The Cepheid channel is directly measured; the SN stretch channel is a falsifiable prediction of the same transport mechanism. The data tell a consistent story across multiple scales. The strongest evidence comes from single-galaxy differential tests: internal radial Period--Luminosity gradients within M31 ($\Delta W = +0.630 \pm 0.195$ mag, $3.24\sigma$ under HST PHAT spatial matching) and the LMC ($\Delta W = +0.0284 \pm 0.0086$ mag, $3.30\sigma$ from OGLE-IV) independently detect the predicted clock-gradient signal free from host-to-host peculiar velocity systematics. At the cosmological level, the redshift-only WLS regression in $H_0$ space across 33 Hubble-flow hosts yields $\kappa_{\rm Cep} = (0.452 \pm 0.220) \times 10^6$ mag ($2.05\sigma$ at $\sigma_v = 150$ km/s), with the joint multi-block likelihood returning $1.58\sigma$ as a consistency check. The supernova $X_i$-step emerges at $1.39\sigma$, also in the predicted direction, placing the longstanding host-mass residual in a concrete, falsifiable timescale-bias framework. These multi-scale detections align in both direction and order of magnitude to span the gap. The definitive per-host recalibration is left to the companion TEP-H0 analysis.
...
The potential of an arbitrary high-redshift SN host does not enter this chain. The Cepheid channel affects only the Cepheid calibrators that set $M_B$, not the SNe themselves. The SN stretch channel (Section 9.5) operates independently on the Hubble-flow SNe.
...
where $\kappa_P$ is the dimensionless period-response coefficient (governing Cepheid period contraction for $X_i > 0$ in massive hosts), $b \approx -3.26$ is the near-infrared Leavitt law slope, and $\kappa_{\rm Cep}^{\rm equiv} = (0.365 \pm 0.304) \times 10^6\ {\rm mag}$ is the Cepheid-channel response coefficient from the endpoint slope conversion in TEP-H0 (Paper 11, Section 4). Here $X_i = (U_i - U_{\rm ref}) / c^2$ is the dimensionless screened potential coordinate, with $U_i = u_\phi^2$ the rotation-based potential proxy and $U_{\rm ref} = (87.165\ {\rm km\,s^{-1}})^2$ the anchor reference potential. The correction is positive for massive calibrators: Cepheid distances are compressed, so the true distance modulus is larger than the observed value.
...
The TEP correction is applied to the Cepheid calibrator hosts with measured rotation velocities from the step_01 potential catalog. The mean correction over these calibrators determines the zero-point shift $\Delta M_B$:
...
The indicator-specific distance divergence (Section 5) is physically suggestive ($3.30\sigma$ in the full CF4 sample) but is dataset- and reduction-dependent ($0.20\sigma$ in the R22-matched subset — a sample-size constraint, not a vanishing effect, as the central values match the TEP-predicted $\sim 0.02$–$0.05$ mag with stable sign). The two-channel structure (Table 7) shows the Cepheid channel (measured) and the SN stretch channel (predicted), each bounded by an independent observational constraint. The falsifiable prediction — the $X_i$-step in Pantheon+ Hubble residuals beyond the SALT3 mass correction — is the subject of Section 10.

## 10. Falsification Pathways and Future Observational Tests

The value of a physical theory lies not only in what it explains but in what it forbids. The TEP framework makes specific, quantitative predictions that can be tested with current and near-future observational facilities. This section outlines three primary falsification pathways, each designed to either confirm or refute the TEP acoustic clock bias hypothesis against the kinematic void alternative.
...
### 10.2 Pathway II: Peculiar Velocity Recalibration with TRGB Distances

The second falsification pathway directly tests the TEP interpretation of the CosmicFlows-4 bulk-flow anomaly as a calibration-sensitive quantity. The indicator-specific distance divergence (Section 5) shows $3.30\sigma$ in the full CF4 catalog but only $0.20\sigma$ in the R22-matched subset — the result is dataset/reduction dependent and physically suggestive but statistically constrained by the small sample size. The calibration sensitivity analysis (Section 6) shows that this compression introduces a distance-dependent shift of $-3.2 \times d$ km/s into the peculiar velocity field. The velocity catalog is calibration-sensitive, while the measured bulk-flow amplitude changes by $3.3\%$; this bounds the scale of calibration leakage into the bulk-flow statistic. A dedicated test would expand the TRGB distance coverage in the CosmicFlows-4 catalog and recompute the bulk flow using only TRGB-calibrated distances.

- The void model predicts the $4.8\sigma$ bulk-flow anomaly persists, because the physical motion of the galaxies is independent of the distance indicator used.

- The TEP framework predicts the velocity catalog is calibration-sensitive, with the bulk-flow amplitude changing by a bounded amount when the distance indicator is switched from Cepheids to TRGB.

The preliminary result in Section 6 shows a 3.3% change in the TRGB-calibrated bulk flow relative to the Cepheid-calibrated value. While the effect is modest, it is in the direction predicted by TEP and is a direct consequence of the indicator divergence (Section 5). A dedicated TRGB distance survey would tighten this constraint by expanding the TRGB-calibrated subsample from 446 galaxies to several thousand, enabling a more precise bulk-flow comparison.
...
The SN light-curve stretch channel (Section 9.6) predicts that Hubble residuals after SALT3 standardization should correlate with the potential coordinate $X_i$ of the SN host, beyond the standard host-mass correction. This is directly testable with Pantheon+ data. A preliminary test (step_45) used the 1,470 Pantheon+ Hubble-flow SNe ($z > 0.01$) with host stellar mass measurements. The host potential was estimated from stellar mass via the baryonic Tully-Fisher relation ($V_{\rm rot} \approx 200\,(M_*/10^{10.5}\,M_\odot)^{1/4}\ {\rm km\,s^{-1}}$), from which $X_i$ was computed using the same anchor reference as the Cepheid analysis. The results are:

The standard mass step (at $\log_{10}(M_*/M_\odot) = 10.5$) is $-1.0 \pm 8.8$ mmag ($0.11\sigma$) in this sample — consistent with the literature value of $\sim 0.04$–$0.06$ mag but underestimated because $\mu_{\rm SH0ES}$ already incorporates a mass-step correction in the SALT3 calibration). The $X_i$ step ($X > 0$ versus $X \le 0$) is $-12.1 \pm 8.7$ mmag ($1.39\sigma$) in the TEP-predicted direction (high-$X$ hosts show more positive Hubble residuals, i.e., appear fainter after standardization). After removing the mass-step component, the residual $X_i$ step is $-9.4 \pm 8.7$ mmag ($1.08\sigma$) — still in the predicted direction but not statistically significant.

An improved analysis (step_48) uses measured $V_{\rm rot}$ from the HyperLEDA catalog (VII/237, VII/238) queried via the Vizier HTTP API for all 1,543 Pantheon+ hosts. Measured rotation velocities (with inclination corrections from axis-ratio data) are obtained for 173 SNe: 33 calibrators and 140 Hubble-flow hosts. The remaining 1,370 Hubble-flow hosts retain the Tully-Fisher proxy. The full sample ($N = 1,470$ Hubble-flow) shows a step of $-16.9 \pm 15.2$ mmag ($-1.11\sigma$) — correct sign, consistent with the Tully-Fisher-only result. The measured-$V_{\rm rot}$-only subsample ($N = 122$ Hubble-flow) shows a step of $+144.7 \pm 59.2$ mmag ($+2.44\sigma$) — the *wrong* sign. The mass-corrected $X_i$ coefficient for this subsample is $+1.49 \times 10^5 \pm 9.18 \times 10^4$ ($1.62\sigma$) — still positive, confirming that the apparent step is driven by sample composition rather than a genuine $X_i$ dependence.

The measured-$V_{\rm rot}$ subsample is highly unbalanced ($N_{\rm high} = 97$, $N_{\rm low} = 25$) and the low-$X_i$ subset is concentrated at very low redshift (median $z = 0.016$, maximum $z = 0.064$) where peculiar velocity contamination ($\sim$100–200 mmag) dominates the Hubble residual. HyperLEDA preferentially samples nearby massive spirals, introducing a selection bias that correlates with $X_i$. The Hubble-flow $X_i$-step with measured $V_{\rm rot}$ is therefore inconclusive because the small, nearby, unbalanced subsample does not provide a clean test. A definitive test requires a deeper $V_{\rm rot}$ catalog (e.g., SPARC, ALFALFA $\alpha$.40, or broader HyperLEDA coverage), populating the low-$X_i$ bin with Hubble-flow SNe at $z > 0.03$ where peculiar velocities are subdominant. The DHOST derivation (Section 4.6) additionally predicts that $\bar\epsilon_0$ should scale as $V_{\rm rot}^2/r^2$ rather than $V_{\rm rot}^2$ alone — a testable distinction that requires Cepheid galactocentric radii from JWST imaging. The void model predicts no $X_i$-step (the bulk flow is independent of host potential).

### 10.5 Additional Tests and Cross-Checks

Beyond the four primary falsification pathways, several additional tests provide further discrimination:
...
Band-dependence of the Cepheid-TRGB offset (Pathway V): The disformal mechanism (Section 4.7) predicts that the TEP correction scales with the Leavitt law slope $|b|$, which differs between optical ($b_V \approx -2.76$) and near-infrared ($b_H \approx -3.26$) bands. The inter-band differential $\Delta\mu_{H-V} = (|b_H| - |b_V|)\,\kappa_P\,X_i \approx 0.5\,\kappa_P\,X_i$ should correlate positively with $X_i$: NIR distances should be more heavily compressed at deeper potentials. This prediction is specific to the disformal channel; conventional systematics (extinction, metallicity, crowding) do not naturally produce this band-dependent spatial signature.

A matched-host test (step_49) is conducted using the Madore & Freedman (2023, arXiv:2309.10859) compilation, which provides both $W(H,VI)$ NIR and $W(V,VI)$ optical Cepheid true distance moduli for 20 galaxies analysed with identical methodology, photometry, and Cepheid samples by the same team — isolating the purely band-dependent effect. The regression of $\Delta\mu_{\rm band} = \mu_{\rm NIR} - \mu_{\rm optical}$ on $X_i$ yields a slope of $+8.1 \times 10^5 \pm 1.5 \times 10^5$ ($5.35\sigma$, positive). The formal-error slope has the TEP-predicted sign: NIR distances are more compressed at deeper potentials. The TEP-predicted slope is $\sim 1.8 \times 10^5$ (using the default $\kappa_P$); the observed slope is $\sim 4.5\times$ steeper, at $4.15\sigma$ tension with the prediction. The high $\chi^2/{\rm dof} = 5.19$ indicates substantial intrinsic scatter not captured by the formal distance errors, and the weak Pearson correlation ($r = 0.19$, $p = 0.42$) suggests the regression is driven by a few leverage points rather than a tight global trend. A secondary sample (Key Project optical versus R22 NIR, $N=9$, cross-team) yields a negative slope at $0.77\sigma$ — not significant, with cross-team systematics limiting diagnostic power. The primary sample result is a qualitative confirmation of the directional sign alongside a quantitative rejection of the baseline amplitude: the formal-error regression has the predicted positive sign ($5.35\sigma$ under the quoted measurement errors), but the large intrinsic scatter ($\chi^2/{\rm dof} = 5.19$; Pearson $r = 0.19$, $p = 0.42$) reduces the hierarchical evidence for a positive slope to $87\%$. The observed amplitude exceeds the TEP prediction by a factor of $4.5$, a $4.15\sigma$ tension with the predicted slope. This constitutes a quantitative rejection of the baseline parameterization at the amplitude level, even as the directional sign is confirmed under the formal error model. The discrepancy indicates either severe unmodeled intrinsic scatter ($\sigma_{\rm int} \approx 0.12$ mag, confirmed by the step_51 Bayesian hierarchical analysis) or a larger disformal coupling $\bar\epsilon_0$ than the Cepheid-channel fit suggests, or residual metallicity/extinction systematics in the same-team band differential that correlate with potential depth. A Bayesian hierarchical analysis (step_51) with intrinsic scatter modelling finds $\sigma_{\rm int} \approx 0.12$ mag and a slope posterior consistent with the OLS point estimate ($P(\text{slope} > 0) = 87\%$, $P(\text{slope} > \text{predicted}) = 82\%$), confirming that the amplitude discrepancy persists even after accounting for intrinsic scatter.

Tracer-type dependence (Pathway VI): The $q_i$ mechanism (Section 4.4) predicts that the Cepheid period bias depends on the tracer used to measure the host systemic redshift. Hosts with H I 21cm redshifts (disk-weighted, extended) should show $q_i^{\rm clock} \approx 1$ and therefore $\Delta\mu \approx 0$, while hosts with nuclear optical redshifts (bulge-weighted, compact) should show $q_i^{\rm clock} < 1$ and the full $\Delta\mu < 0$ signal. A preliminary classification (step_47) of the 22-galaxy matched sample finds $N_{\rm HI} = 14$ and $N_{\rm nuclear} = 8$. The H I subsample shows $\Delta\mu = -0.092 \pm 0.031$ mag (11/14 shorter), while the nuclear subsample shows $\Delta\mu = -0.059 \pm 0.039$ mag (6/8 shorter). The difference $\Delta\mu_{\rm nuclear} - \Delta\mu_{\rm HI} = +0.032 \pm 0.050$ ($0.64\sigma$) is opposite to the $q_i$ prediction (nuclear should be more negative) but not significant. This null result bounds the conformal $q_i$ channel to zero at the $\sim 0.05$ mag level and is consistent with dominance of a tracer-independent component: the disformal term is tracer-independent by construction, so the absence of tracer dependence is consistent with the disformal mechanism. A definitive tracer-type test requires a larger sample with unambiguous tracer classification from HyperLEDA/NED metadata.

### 10.6 Summary of Falsification Pathways

| Pathway | Observable | Void Prediction | TEP Prediction | Feasibility |
|---|---|---|---|---|
| I | Expanded Cepheid/TRGB matched sample | No indicator offset | Cepheid distances shorter; JWST sample ($N=17$): mean $\Delta\mu = -0.023 \pm 0.022$ ($1.02\sigma$), $\chi^2/{\rm dof} = 0.77$; Anand F090W subset ($N=7$): correct-sign slope at $0.01\sigma$ consistency | JWST TRGB (GO-1995, GO-1685); expand to $\sim 30$ galaxies |
| II | Bulk flow with TRGB distances | Persists at $4.8\sigma$ | Reduced or eliminated | Existing CosmicFlows-4 data |
| III | $q_i$ correlation with Cepheid offset | No correlation | Positive correlation | JWST + IFU spectroscopy |
| IV | $X_i$-step in Pantheon+ Hubble residuals | No $X_i$-step | Full sample ($N=1470$): $-16.9 \pm 15.2$ mmag ($-1.11\sigma$), correct sign; measured-$V_{\rm rot}$ subsample ($N=122$): $+144.7 \pm 59.2$ mmag ($+2.44\sigma$, wrong sign — driven by selection bias at low $z$); mass-corrected $X_i$ regression: $+1.49 \times 10^5 \pm 9.18 \times 10^4$ ($1.62\sigma$, wrong sign) | Inconclusive with current HyperLEDA coverage; needs deeper $V_{\rm rot}$ catalog |
...
| V | Band-dependence: optical versus NIR Cepheid offset | No band dependence | NIR offset $\sim 18\%$ larger than optical ($b_H \approx -3.26$ versus $b_V \approx -2.76$); matched-host test (MF2023, $N=20$): slope $+8.1 \times 10^5 \pm 1.5 \times 10^5$ ($5.35\sigma$, positive — correct sign); amplitude $4.5\times$ steeper than predicted, $\chi^2/{\rm dof} = 5.19$ | Sign correct under formal errors ($5.35\sigma$ OLS); hierarchical evidence for positive slope $87\%$; amplitude quantitatively rejected ($4.5\times$ excess, $4.15\sigma$ tension); discrepancy persists under Bayesian hierarchical modelling (step_51: $\sigma_{\rm int} \approx 0.12$ mag) |
| VI | Tracer-type dependence: H I versus nuclear $z$ | No tracer dependence | H I-anchored hosts show $\Delta\mu \approx 0$; nuclear-anchored show $\Delta\mu \lt 0$; preliminary test: null ($0.64\sigma$, opposite to prediction) — bounds $q_i$ channel to zero, consistent with disformal dominance | Needs larger sample with unambiguous tracer classification |

Each of these pathways provides a distinct falsification test of the TEP framework against the kinematic void hypothesis. The first two can be performed with existing or near-future data; the third requires new observations but is feasible with current facilities. Together, they constitute a comprehensive falsification program that can definitively distinguish between the two frameworks within the next several years. An aggregate sign-consistency statistic across these auxiliary pathways, together with a named proxy-versus-rigorous-subset pattern recurring across the Xi regression and the $X_i$-step test, is discussed in Section 11.8.
...
- Screening mechanism: The screening functions used in the TEP pipeline are parameterized models for the environmental suppression operator $\mathcal{S}_\Sigma(\mathcal{E})$ posited in the foundational paper (Paper 0). A derivation from a specific scalar potential $V(\phi)$ with a demonstrated chameleon or Vainshtein solution is a theoretical goal of the TEP programme.
...
A note on the distance-redshift relations used here is in order. The analysis uses standard flat-$\Lambda$CDM luminosity distance relations to invert Pantheon+ distance moduli for $H_0(z)$. The foundational TEP paper (Paper 0) proposes a non-standard cosmological architecture in which redshift is interpreted as conformal temporal shear ($1+z = A_0/A_{\rm em}$) rather than FLRW scale factor expansion. Standard FLRW distance variables are used here as the common operational framework because both the Pantheon+ products and the published KBC $H_0(z)$ predictions are expressed in that conventional framework. The present comparison tests the KBC prediction on its own observational coordinates and does not require an assumption that FLRW expansion is the fundamental TEP cosmology. A self-consistent TEP cosmological distance-redshift relation, developed in companion papers (TEP-HUB, Paper 30; TEP-BBN, Paper 29), would be required for a fully rigorous analysis at higher redshifts.

### 11.5 The Screening Mechanism and Local Tests

A critical question for any scalar-tensor modification is compatibility with local tests of the Equivalence Principle. The TEP scalar field exhibits a screening mechanism analogous to chameleon or symmetron theories: in regions of very high matter density (the solar neighborhood, terrestrial laboratories), the field is screened and deviations from standard gravity are suppressed. This phenomenological screening structure suppresses the observable shear in the local regime; the microscopic completion must reproduce that screened limit, ensuring full compatibility with local tests of gravity, atomic clock comparisons, and lunar laser ranging.

The active regime—where the scalar field produces observable temporal shear—is the dilute regime: regions where the matter density is low enough that the field is not screened, but deep enough into a gravitational potential well that the gradient of $\phi$ is significant. The interiors of massive spiral galaxies, particularly the inner disks where most Cepheids are observed, fall precisely into this regime. The shallow-potential calibrators (LMC, SMC, Milky Way Cepheids in the solar neighborhood) are either screened or have negligible $\phi$ gradients, establishing the baseline against which the deep-potential hosts show the bias.

This screening structure makes the TEP framework testable and falsifiable: the effect is not universal (which would have been detected in local tests) but appears specifically in the transition from shallow to deep gravitational environments. The companion paper TEP-EXP (Paper 9) develops the measurement taxonomy that distinguishes these regimes and identifies the proximity-regime screening transition as a key discriminating observable.
...
- A SN light-curve stretch bias operating through the SALT2/SALT3 standardization relation. The light-curve stretch $x_1$ is a diffusion timescale — a clock — and inherits an environment-dependent bias under TEP. This channel (Section 9.6) provides a mechanism for the unexplained $\sim 15$-year-old SN Ia host-mass step anomaly and is a falsifiable prediction of the same transport mechanism (Table 7). A preliminary test (step_45) on 1,470 Pantheon+ Hubble-flow SNe finds an $X_i$-step of $-12.1 \pm 8.7$ mmag ($1.39\sigma$) in the TEP-predicted direction, reducing to $-9.4 \pm 8.7$ mmag ($1.08\sigma$) after mass-step correction. An expanded analysis with measured $V_{\rm rot}$ from HyperLEDA (step_48, $N=122$ Hubble-flow) is inconclusive due to severe selection bias (the low-$X_i$ bin is concentrated at $z < 0.06$ where peculiar velocities dominate). A definitive test requires a deeper $V_{\rm rot}$ catalog that populates the low-$X_i$ bin with Hubble-flow SNe at $z > 0.03$.
...
The evidence converges on five points:

- The published KBC $H_0(z)$ curves are decisively rejected by the full Pantheon+ covariance ($\Delta{\rm AIC} > 100$ over the valid domain, $>190$ over the full sample). When allowed to vary, the void family collapses to a flat limit ($\Delta H_0^{\rm ML} = 0$). The calibration-independent ratio $R_H$ excludes the predicted decline at $>7.7\sigma$.

- Indicator comparisons show the directional signature expected from acoustic-clock bias and forbidden by pure kinematics ($2.39\sigma$ sign test; Xi regression negative on the primary analysis and on the independent CF4 non-R22 subset; $\kappa_{\rm Cep} = (0.452 \pm 0.220) \times 10^6$ mag, $2.05\sigma$ from redshift-only WLS; $1.58\sigma$ from joint multi-block; M31 $3.24\sigma$ and LMC $3.30\sigma$ internal gradients).

- The Cepheid channel reconstruction (Paper 11, Step 04) yields $H_0 = 66.65 \pm 1.58\ {\rm km\,s^{-1}\,Mpc^{-1}}$, consistent with Planck at $0.45\sigma$. TEP additionally predicts an independent SN light-curve stretch channel that provides a mechanism for the host-mass step, which has remained unexplained for $\sim$15 years, and is consistent with the Jensen et al. (2025) TRGB-anchored SBF result. The SN channel is falsifiable through the $X_i$-step test.

- The conformal term cannot carry the effect without violating spectroscopic line-shift bounds. Instead, the disformal metric sector provides the spatial shear and synchronization holonomy necessary to translate the core-disk clock differential ($q_i^{\rm clock} < 1$) into an observing-chain bias. The conformal sector satisfies the relevant spectroscopic/Cassini bound; the effective galactic disformal coupling is not subject to that same leading conformal constraint, while complete local-constraint matching depends on the screened $M_*$ completion.

- The remaining open issues are: the $2.05\sigma$ redshift-only Cepheid coupling, independent replication of the M31 and LMC internal gradients, the inconclusive $X_i$-step with measured $V_{\rm rot}$ (selection-biased subsample), the DHOST-to-phenomenology mass-scale matching, and the need for measured rotation velocities for the full Hubble-flow sample. The falsifiable predictions — band-dependence (Pathway V, sign correct under formal errors at $5.35\sigma$ OLS but hierarchical evidence reduced to $87\%$ by intrinsic scatter; amplitude quantitatively rejected at $4.15\sigma$ tension, $4.5\times$ excess), tracer-type dependence (Pathway VI, null — consistent with disformal dominance), the JWST matched sample ($N=17$, mean $\Delta\mu = -0.023$, $1.02\sigma$, $\chi^2/{\rm dof} = 0.77$), the $X_i$-step with measured $V_{\rm rot}$, and the $V_{\rm rot}^2/r^2$ scaling of $\bar\epsilon_0$ — are testable with existing and near-future data.

The Hubble tension, in this interpretation, is not a crisis of cosmic expansion but an artifact of observation. The evidence is consistent with a two-channel clock-calibration bias. It derives from a Horndeski/kinetic-braiding scalar-tensor completion with specified coupling functions $A(\phi) = e^{\beta_A\phi/M_{\rm Pl}}$ and $B(\mathcal{X}) = (\beta_B/M_*^4)\,F(\mathcal{X}/M_*^4)$, yielding a consistency relation for $\bar\epsilon_0$ that is compatible with Cassini solar-system bounds; the DHOST natural-unit to mass-scale matching remains unresolved. The disformal transport mechanism is dominated by the spatial and time-space sectors of the disformal metric; it does not generate the leading conformal spectroscopic signature and is structurally distinct from any known systematic. The band-dependence test returns the predicted positive sign under the formal error model ($5.35\sigma$ OLS), though large intrinsic scatter reduces the hierarchical evidence to $87\%$; the JWST matched sample shows the predicted negative mean $\Delta\mu$ with scatter consistent with the model; the $X_i$-step is inconclusive due to catalog depth limitations. The quantitative development of the two channels and the definitive radial-gradient tests are given in the companion TEP-H0 paper. The Hubble tension is not a crisis of cosmic expansion, but the inevitable consequence of treating a dynamical time field as a static coordinate. The astronomical observations are flawless; the stars are keeping perfect time in their local frames. The tension is simply the artifact of enforcing universal isochrony on a universe governed by dynamic proper time.
...
Watkins, R., Allen, T., Bradford, C. J., et al. (2023). "Analysing the large-scale bulk flow using cosmicflows4: increasing tension with the standard cosmological model." *Monthly Notices of the Royal Astronomical Society*, 524, 1885--1892. $4.8\sigma$ bulk-flow tension with ΛCDM at $200\ h^{-1}\ {\rm Mpc}$.
...
- *scripts/steps/*—Reproducible analysis pipeline with 21 active step scripts organized into four blocks (data ingestion, indicator divergence and void falsification, void boundary test, and TEP reconstruction / synthesis).
...
This executes all 21 active step scripts in sequence, populating `results/figures/` and `results/outputs/` with fresh data. Individual blocks or steps can be run selectively:
...
The pipeline is fully implemented and all 21 steps run successfully against real published data. The pipeline structure, data sources, and analysis methodology are documented in `scripts/README.md`.
...
This generates the static site in `site/dist/` and the markdown version at the repository root (`31-TEP-VOID-v0.1-Valencia.md`). A PDF can be generated from the built site:
