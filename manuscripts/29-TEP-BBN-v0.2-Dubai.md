...
Version: v0.2 (Dubai)
First published: 12 August 2026 - Last updated: 28 August 2026
...
Under a shared Voigt architecture and Monte-Carlo calibrated nested testing, the statistical preference for a true-D model over unrestricted hydrogen is sensitive to the choice of parent component and is not uniform across the three best-studied D/H quasar sightlines. Isotope identifiability is found to be *controlled by both Doppler regime and D column density*: at the actual Q1009 D column ($N_{\rm D} = 5.7\times10^{12}\,\mathrm{cm}^{-2}$), the maximum single-pixel discrepancy is $0.52\sigma$ for thermally-dominated gas at Keck/HIRES resolution, falling to $0.10\sigma$ for the fitted mixed-regime parameters ($b_{\rm turb}/b_{\rm therm} \approx 1.87$) and $0.0002\sigma$ for turbulence-dominated gas. At the unsaturated LLS-class columns typical of D/H measurements, the intrinsic H/D profile-shape difference is sub-noise on a per-pixel basis; isotope identifiability must therefore be assessed per absorber as a function of both column and thermal state, and cumulative discrimination across many pixels and transitions can still be substantial. A nested-hypothesis analysis of three benchmark sightlines---Q1009+2956 (Keck/HIRES, $z=2.5042$), PKS\,1937$-$101 (ESPRESSO, $z=3.572$), and J1332$+$0052 (UVES+HIRES, $z=3.421$)---with the shared Voigt architecture held fixed across hypotheses, finds that the *parent-reassignment statistic*---which gives D maximum freedom to select the most favorable parent ---is the more conservative and informative number. Under the conservative $\pm 1000$\,km/s parent window with $N = 1000$ Monte Carlo realisations, $T_{\rm parent}$ and $p_{\rm parent}$ are ($3.42$, $0.094$) for Q1009+2956, ($30.63$, $0.002$) for PKS\,1937$-$101, and ($0.43$, $0.359$) for J1332$+$0052. For Q1009+2956, a physically motivated window ($\pm 3b_{\rm max} = \pm 170$\,km/s, scaled to the absorber's own velocity structure) was also run with $N = 200$ realisations, giving $p_{\rm parent} = 0.119$ ($p_{\rm std} = 0.005$) --- consistent with the conservative-window result and confirming that the wider window was not driving the non-significance (robustness check C1, Section 5.5). Physical-window Monte Carlos for PKS\,1937$-$101 and J1332$+$0052 are deferred. The canonical-parent (standard) statistic yields stronger discrimination ($T=24.04$, $p_{\rm std}=0.001$ for Q1009; $T=85.41$, $p=0.002$ for PKS\,1937) but reflects the specific published parent assignment. After Bonferroni correction for the six-test family (2 statistics $\times$ 3 sightlines, threshold $\alpha/6 = 0.0083$), PKS\,1937$-$101 $p_{\rm parent} = 0.002$ and Q1009+2956 $p_{\rm std} = 0.001$ survive; Q1009+2956 $p_{\rm parent} = 0.094$ does not. The offset-exactness test (Gate 4E) shows that the fitted free-H velocities deviate from the isotope-shifted position by $+1.1$\,km/s (Q1009), $+5.7$\,km/s (PKS\,1937), and $-20.5$\,km/s (J1332), providing a per-sightline discriminator. The velocity-selection theorem (Gate 6) explains the universality of the $-81.6$\,km/s offset as a selection effect of the D-search protocol; the quantitative population forecast (Gate 6-II) predicts $\sim 16$ orphan narrow components at non-isotope offsets across $\sim 80$ high-S/N LLS sightlines, testable with existing archival data. A multi-pronged evidence analysis (Gate 8) evaluates three discriminators---b-value ratio, velocity offset deviation, and column density difference---calibrated against a true-D injection Monte Carlo. The individual statistics are not significant after calibration, but the joint Mahalanobis statistic is significant for Q1009$+$2956 and PKS\,1937$-$101 ($p < 0.033$), indicating that the combination of fitted free-H parameters is anomalous relative to the true-D null. J1332$+$0052 is not significant in the joint statistic ($p = 0.13$). The three sightlines exhibit the predicted monotonic ordering of offset deviations with $N_{\rm HI}$ (Spearman $\rho = 1.0$), though with $N = 3$ the sample is too small for the correlation itself to constitute significant evidence. The cross-sightline variance---significant at one sightline under the conservative statistic, at two under the standard statistic, and not significant at the third---is consistent with the TEP framework's prediction that temporal shear depends on absorber-specific core-edge geometry; a cross-sightline amplitude ranking is computed in Section 3.3. A pilot orphan search protocol (Gate 6-III) on $10$--$15$ archival sightlines from SQUAD, UVES SLS, and ESPRESSO surveys would provide a first observational test of the population forecast. The broader TEP corpus establishes CMB acoustic peak preservation at $< 6$\,ppm (TEP-HC), temporal-horizon regularity (TEP-TH), cosmological distances without expansion (TEP-C0), and JWST high-redshift anomaly resolution (TEP-JWST).

The Temporal Equivalence Principle (TEP) provides a concrete alternative mechanism for the deuterium-like velocity feature. The TEP absorber field deterministically predicts the blueward sign from conformal core-edge geometry; the environmental screening operator $\mathcal{S}_\Sigma(|\Phi|/c^2)$ is calibrated by three boundary conditions (absorber, Cassini, galactic halo) with $\Phi_{\rm half} = 1.59 \times 10^{-7}$ and $n = 3.56$; the conformal-only amplitude is quantified ($|\Delta v_{\rm conf}| \approx 0.006\text{ km/s}$ for $R = 30$ kpc with uniform-sphere ionisation correction), and the required amplification $\mathcal{A}_{\rm env} \approx 1.4 \times 10^4$ is identified as a falsifiable target for the non-perturbative transport path $\mathcal{C}_{T,\parallel}$. Cosmological redshift is formulated as temporal transport over a static spatial background, separating observed temperature, chronology and photon energy from the local thermodynamic history that governs nuclear processing. A temporal-exposure convergence condition ($p + q > 1$) ensures that the integrated astration exposure is finite for any individual worldline, and the helium-4 mass fraction ($Y_{\rm eq} = 0.247$) emerges as the equilibrium of baryonic-cycling reaction flows under temporal-horizon metal sequestration for the adopted stellar histories. In the static spatial geometry, the line-of-sight optical depth diverges at high redshift under an asymptotic electron-density condition, creating an observable boundary without a physical plasma wall.
...
| --- | --- | --- | --- | --- | --- |
| $M_D$ | D tied to parent H | 3 | $-16595.32$ | 0 | — |
| $M_{H,\rm free}$ | unrestricted H | 4 | $-16583.30$ | $-22.04$ | observationally embeds $M_D$ at Q1009 precision |
| $M_{D+H}$ | D + unrestricted H | 5 | $-16583.30$ | $-20.04$ | contains $M_{H,\rm free}$ if $N_{\rm D}\to 0$ (verified to $2.8\times10^{-10}$ in $\Delta\ln L$) |
...
Against the most advantageous alternative parent assignment, the free-H interpretation is still preferred for Q1009+2956, but by a far smaller margin: $T_{\rm parent} = 3.42$, compared with $T = 24.04$ for the canonical parent. Most of the nominal discrimination therefore reflects the specific published parent assignment rather than an intrinsic inability of hydrogen to reproduce the feature, and the parent-reassignment statistic is the conservative quantity to calibrate. When calibrated inside the true-D Monte Carlo loop—utilizing bounded L-BFGS-B optimization with a multi-start grid spanning the full velocity range $[-160, +50]\text{ km/s}$, $\log N_{\rm D} \in [0, 20]$, $T_K \in [100, 4\times10^4]$\,K, and $b_{\rm turb} \in [1, 50]$\,km/s, with the best likelihood retained across all starts to prevent local minima from artificially widening the null distribution—the empirical $p_{\rm parent}$ is reported below.

**Parent-reassignment window.** Two parent-reassignment windows are reported. The *conservative* window ($\pm 1000\text{ km/s}$) gives the deuterium hypothesis maximum freedom to find a favorable parent, establishing a lower bound for the $T_{\rm parent}$ statistic. Of the 43 H I components in the Q1009 model, 14 fall within this window; none are DLA-class ($\log N_{\rm HI} > 20.3$), and components from unrelated absorber systems at $v \sim -47\,000$ to $-74\,000\text{ km/s}$ are correctly excluded. The *physical* window ($\pm 3b_{\rm max}$, where $b_{\rm max} = 56.7$\,km/s is the largest Doppler parameter of the main complex) restricts eligible parents to those within the kinematic structure of the absorber, reducing the pool from 43 to 9 components. The physical window is reported as the *headline* statistic because it is motivated by the absorber's own velocity structure rather than an arbitrary fixed range. Under the physical window, the conclusions are essentially unchanged from the conservative window (see robustness check C1 in Section 5.5), confirming that the $\pm 1000\text{ km/s}$ conservative bound was not driving the results.

A critical interpretation issue arises in the parent-reassignment test. For all three sightlines, $T_{\rm parent} < T_{\rm obs}$, meaning that D tied to an alternative parent achieves a likelihood closer to free-H than D tied to the canonical parent. This indicates that the standard-statistic rejection reflects the specific published parent assignment rather than an intrinsic inability of hydrogen to reproduce the feature. Equivalently, D remains a viable interpretation when allowed a different parent, so the parent-reassignment test does not strongly discriminate between D and H. The parent-reassignment statistic is therefore the conservative quantity to calibrate, and the $p_{\rm parent}$ values reported below should be interpreted as the significance under the most favorable parent assignment for D.

**Parent-identity diagnostic (Gate 7).** Under the isotope-D interpretation, the best alternative parent $j^*$ (the H I component maximizing $\ln L(M_D \mid j)$) should be the main narrow thermal component — D is tied to the gas it formed beside. Under TEP, $j^*$ might be a diffuse, low-column edge component — the edge gas anchors to its own local structure. For Q1009+2956, the best alternative parent is component 10 ($v = -50.8$\,km/s, $\log N_{\rm HI} = 13.91$, $b = 41.2$\,km/s) — a diffuse, high-$b$ edge component, not the main narrow thermal component (components 0–2, $\log N_{\rm HI} = 17.36$, $b \approx 18$\,km/s). This is consistent with the TEP-predicted signature, but the parent-identity test is presently treated as a descriptive diagnostic rather than a TEP-specific discriminator. Under TEP the feature is ordinary edge H, not D tied to any parent; the identity of the parent that minimizes a mis-specified D fit may be a fitting artefact rather than a physical TEP observable. A derivation showing why a displaced edge-H feature mathematically causes the false-D likelihood to maximize on diffuse edge components is required before Gate 7 can be elevated to a TEP-specific prediction.
...
| --- | --- | --- | --- | --- | --- |
| Q1009+2956 | $-52.4$ | $-81.6$ | $-134.0$ | $-132.9$ | $+1.1$ |
| PKS\,1937$-$101 | $+8.7$ | $-81.6$ | $-72.9$ | $-67.2$ | $+5.7$ |
| J1332$+$0052 | $-17.4$ | $-81.6$ | $-99.0$ | $-119.5$ | $-20.5$ |

Q1009+2956 sits $1.1$\,km/s from the isotope shift — consistent with isotope-D to within the spectral resolution. PKS\,1937$-$101 deviates by $5.7$\,km/s — marginally consistent. J1332$+$0052 deviates by $-20.5$\,km/s — significantly offset from the isotope shift, consistent with the TEP prediction that the edge-gas feature need not sit at the exact isotope-shifted velocity. The cross-sightline pattern — decreasing offset-exactness with increasing D column — is quantitatively testable: if the feature is isotope-D, all three should sit at $v_{\rm iso}$ within spectral resolution; under TEP, the offset varies with core-edge geometry. The J1332 deviation is the most TEP-consistent result, and it is also the sightline with the highest D column (saturated, $\log N_{\rm D} \approx 14.2$), where the edge-gas interpretation is most physically motivated.
...
| --- | --- | --- | --- |
| C1x1 coadd | coadd | 16.91 | $-7.13$ |
| C1x2 coadd | coadd | 21.64 | $-2.40$ |
| C5x1 coadd | coadd | 15.85 | $-8.19$ |
| C5x2 coadd | coadd | 17.43 | $-6.61$ |
| Ly$\alpha$ | transition | 26.86 | $+2.82$ |
| Ly$\beta$ | transition | 18.52 | $-5.52$ |
| Ly$\gamma$ | transition | 21.97 | $-2.07$ |
| Ly6 | transition | 23.90 | $-0.14$ |
| Ly13 | transition | 24.07 | $+0.03$ |
| Ly14 | transition | 24.05 | $+0.01$ |

The full LOO vector reveals that the discrimination is NOT concentrated in Ly$\alpha$: removing Ly$\alpha$ *increases* $T$ from 24.04 to 26.86, indicating that the 10-parameter Ly$\alpha$ continuum absorbs the broad H I wing and the isotope discrimination shifts to the higher-order Lyman transitions where the continuum is less flexible. Ly$\beta$ is the single most important transition ($T_{-\mathrm{Ly}\beta} = 18.52$, $\Delta T = -5.52$), followed by Ly$\gamma$ ($\Delta T = -2.07$). The high-order transitions (Ly6, Ly13, Ly14) contribute negligibly ($|\Delta T| < 0.15$). The coadd deletions show that the C5 settings (lower resolution, $R\approx37\,000$) contribute more discrimination than the C1 settings ($R\approx49\,000$). This is not explained by resolution alone — a matched-resolution test would be required to establish that. The C5 dominance may instead reflect differences in signal-to-noise ratio, spectral coverage, continuum flexibility, number of informative pixels, or exposure properties between the two coadd settings. The result demonstrates that a benchmark high-redshift D/H system is not spectroscopically self-authenticating once the displaced-H model class is admitted.
...
**Quantitative population forecast (Gate 6-II).** The observed cross-sightline deviations from the isotope shift ($+1.1$, $+5.7$, $-20.5$\,km/s for Q1009, PKS\,1937, J1332 respectively) imply a shear spread $\sigma_{\rm shear} \approx 11$\,km/s. The D-search selection function $|\Delta v - v_{\rm iso}| < 5$\,km/s captures only $\sim 34\%$ of edge-shear features; the remaining $\sim 66\%$ are orphans absorbed into the H I component structure. In a sample of $\sim 80$ high-S/N LLS sightlines with full Lyman coverage (SQUAD DR1, UVES SLS, ESPRESSO D/H survey), $\sim 24$ shear features are expected, of which $\sim 8$ are identified as D and $\sim 16$ are orphans. The predicted orphan surface density is $\sim 2$ per 10 sightlines at $|\Delta v - v_{\rm iso}| > 5$\,km/s, falling to $\sim 1$ per 10 sightlines at $> 10$\,km/s. Orphan features are predicted to have narrow Doppler parameters ($b < 10$\,km/s, cool edge gas), low columns ($\log N_{\rm HI} \sim 12$--$14$), and thermal Doppler regimes — distinguishable from the normal H I population by an excess of narrow, low-column components at non-isotope offsets. Under the isotope-D null, no such excess is expected. This is a targetable observational program using existing archival data.

The embedding pipeline has been generalised to support multiple sightlines via a sightline-configuration system. Two additional benchmark D/H quasar sightlines have now been analysed with the identical nested-hypothesis framework, providing the first cross-sightline application of the TEP spectroscopic methodology. The heterogeneous outcome is qualitatively consistent with TEP's prediction that temporal shear depends on absorber-specific core-edge geometry; an a priori amplitude ranking remains to be calculated.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Q1009$+$2956 | 2.5042 | Keck/HIRES | 24.04 | 3.42 | 0.001 (0/1000) | 0.094 (93/1000) | Mixed ($b_{\rm turb}/b_{\rm therm} \approx 1.87$, $0.10\sigma$) | Not significant; D viable with alternative parent |
| PKS 1937$-$101 | 3.572 | ESPRESSO | 85.41 | 30.63 | 0.002 (1/1000) | 0.002 (1/1000) | Thermal | Significant; free-H preferred even under most favorable parent |
| J1332$+$0052 | 3.421 | UVES+HIRES | 0.49 | 0.43 | 0.295 (294/1000) | 0.359 (358/1000) | Thermal, saturated (no Ly$\alpha$ coverage) | Not significant; no preference either way |

$^{\dagger}$ For J1332$+$0052, the canonical-parent standard statistic is reported as the primary $p_{\rm std}$ for consistency with Q1009 and PKS\,1937. The fair standard statistic (simulated D best-of-parents vs observed canonical-parent) gives $p_{\rm std,fair} = 0.437$ (436/1000). Both definitions yield non-significant results for J1332$+$0052; the canonical-parent value is reported in the table for cross-sightline uniformity.

Under the conservative parent-reassignment statistic, the results are: for Q1009+2956, $p_{\rm parent} = 0.094$ (93/1000 exceedances), not significant—D remains a viable interpretation when allowed an alternative parent. For PKS\,1937$-$101, $p_{\rm parent} = 0.002$ (1/1000 exceedances), significant—free-H is preferred even under the most favorable parent assignment. For J1332$+$0052, $p_{\rm parent} = 0.359$ (358/1000 exceedances), not significant—no preference either way. The standard (canonical-parent) statistic yields stronger discrimination for Q1009+2956 ($p_{\rm std} = 0.001$, 0/1000) and PKS\,1937$-$101 ($p_{\rm std} = 0.002$, 1/1000), but reflects the specific published parent assignment. The cross-sightline variance is consistent with the TEP framework, in which temporal shear depends on localized core-edge geometry. A cross-sightline amplitude ranking from the density-weighted Green function $K$ is computed in Section 3.3 (Gate 5).

**Multiplicity correction.** Six tests are reported (2 statistics $\times$ 3 sightlines). Applying the Bonferroni correction (threshold $\alpha/6 = 0.0083$), two results survive: PKS\,1937$-$101 $p_{\rm parent} = 0.002$ and Q1009+2956 $p_{\rm std} = 0.001$. Q1009+2956 $p_{\rm parent} = 0.094$ does not survive correction. The unambiguous empirical headline is therefore: PKS\,1937$-$101 shows a significant free-H preference that survives both parent reassignment and multiplicity correction; Q1009+2956 shows a significant preference only under the canonical-parent statistic; J1332$+$0052 shows no preference under either statistic.
...
| --- | --- | --- | --- | --- | --- |
| Q1009+2956 | 17.36 | 20.61 | 0.006 | $\sim 1.4\times10^4$ | $-81.6$ (anchor) |
| PKS\,1937$-$101 | 17.89 | 20.87 | 0.011 | $\sim 7.6\times10^3$ | $\sim -100$ km/s |
| J1332$+$0052 | 18.50 | 21.18 | 0.022 | $\sim 3.8\times10^3$ | $\sim -200$ km/s |

A universal $\mathcal{A}_{\rm env}$ — which is what a geometry-independent transport path $\mathcal{C}_{T,\parallel}$ most naturally provides — is excluded at face value: the predicted offsets would be $-55$, $-100$, and $-200$\,km/s, varying by a factor of $\sim 4$ across sightlines, whereas all three observed features cluster at $-81.6$\,km/s. Two interpretations are consistent with this:
...
The falsification criterion is restated per sightline: the $\mathcal{A}_{\rm env} > 10^3$ floor becomes $\mathcal{A}_{\rm env} > 10^3$ for Q1009+2956, $\mathcal{A}_{\rm env} > 10^3$ for PKS\,1937$-$101, and $\mathcal{A}_{\rm env} > 10^3$ for J1332$+$0052. The population prediction — that edge-shear features at offsets other than $-81.6$\,km/s exist and are being absorbed into H I component structure — is the discriminating observational test.
...
- **Conformal screening: three-anchor calibration closes Gate 10.** The environmental operator $\mathcal{S}_\Sigma(\mathcal{E})$ is multi-variable [1]; the potential-depth projection $\mathcal{S}_\Sigma(|\Phi|/c^2) = 1/(1 + (|\Phi|/c^2 / \Phi_{\rm half})^n)$ is calibrated by three boundary conditions with *corrected* $|\Phi|/c^2$ values: (1) absorber regime $|\Phi_{\rm abs}|/c^2 \sim 10^{-8} \to \mathcal{S}_\Sigma > 0.999$ (unscreened, bare $\beta_A = -1.0$), (2) solar system $|\Phi_\odot|/c^2 = GM_\odot/(R_\odot c^2) = 2.12 \times 10^{-6}$ (Cassini grazing ray at solar radius) $\to \mathcal{S}_\Sigma \approx 10^{-4}$ (Cassini-bound), and (3) galactic halo $|\Phi_{\rm gal}|/c^2 = v_{\rm circ}^2/c^2 = 5.38 \times 10^{-7}$ (solar circle, $v_{\rm circ} = 220\text{ km/s}$) $\to \mathcal{S}_\Sigma \approx 0.013$ (corpus value [54]). The solar-system and galactic potentials differ by $3.94\times$ — earlier drafts incorrectly assumed both were $\sim 10^{-6}$. Solving the three-anchor system yields $\Phi_{\rm half} = 1.59 \times 10^{-7}$ and $n = 3.56$, giving $\mathcal{S}_\Sigma(2.12 \times 10^{-6}) = 1.0 \times 10^{-4}$ (Cassini: $|\gamma_{\rm PPN} - 1| \approx 2 \times 10^{-8}$, PASS), $\mathcal{S}_\Sigma(5.38 \times 10^{-7}) = 0.013$ (galactic: $\beta_{\rm eff} \simeq 0.013$, PASS), and $\mathcal{S}_\Sigma(10^{-8}) = 0.9999$ (absorber: unscreened, PASS). The $3.94\times$ potential-depth ratio raised to $n = 3.56$ yields $132\times$ screening contrast — sufficient to differentiate Cassini from galactic without a second screening variable. Gate 10 is closed: the pure potential-depth law with corrected $|\Phi|/c^2$ values satisfies all three constraints simultaneously. The multi-variable operator $\mathcal{S}_\Sigma(\mathcal{E})$ remains the corpus framework for domains where density and gradient projections are physically relevant (UCD, LLR, LHC), but the potential-depth projection alone is sufficient for the absorber, solar-system, and galactic-halo regimes. The product $\beta_{\rm eff}^2 \times |\Phi|/c^2$ peaks in the unscreened intermediate regime ($|\Phi|/c^2 \sim 10^{-7}$), confirming that the fifth force is strongest in diffuse clouds and suppressed in dense environments.

- **Conformal-only amplitude: quantified.** The Poisson Green function with the bare coupling and the uniform-sphere ionisation-corrected total gas column gives $|\Delta v_{\rm conf}| \approx 0.006\text{ km/s}$ (for $R = 30$ kpc). The required amplification factor $\mathcal{A}_{\rm env} = |\Delta v_T|/|\Delta v_{\rm conf}| = 81.6 / 0.006 \approx 1.4 \times 10^4$. The falsification threshold is set at $\mathcal{A}_{\rm env} < 10^3$ (see scaling analysis below).
...
Under the three-anchor calibration, the solar-circle, globular-cluster, and Earth–Moon entries exceed dynamical fifth-force bounds in the force sector. The resolution is grounded in the matter metric $\tilde{g}_{\mu\nu} = A^2(\phi)\,\eta_{\mu\nu} + B(\phi)\,\partial_\mu\phi\,\partial_\nu\phi$ defined in Section 3.1. The clock shift arises exclusively from the conformal modification to $\tilde{g}_{00}$: for a static scalar field ($\partial_0\phi = 0$), the disformal contribution to $\tilde{g}_{00}$ vanishes identically, so the clock coupling is governed by $\mathcal{S}_\Sigma$ acting on the conformal factor $A(\phi)$ alone. Dynamical fifth-force constraints, by contrast, apply to spatial geodesics and massive particle acceleration, which interact with the spatial components $\tilde{g}_{ij}$ and the disformal/derivative couplings $B(\phi)\,\partial_i\phi\,\partial_j\phi$. The *clock-sector / force-sector split* is therefore not an ad-hoc patch but a structural consequence of the metric decomposition: the corpus phenomenology — Cepheids, flyby, absorber shear, all redshift/clock effects — is governed by $\mathcal{S}_\Sigma$ acting on the clock coupling ($\tilde{g}_{00}$), while the dynamical fifth force is governed by a separately Cassini-bounded sector ($\tilde{g}_{ij}$). This is consistent with the corpus phenomenology, which is exclusively redshift/clock effects and never dynamical. Under this split, the solar-circle, globular-cluster, and Earth–Moon constraints apply to the force sector (which is independently bounded by Cassini), not to the clock sector. The absorber shear — a clock effect — remains unscreened at $|\Phi|/c^2 \sim 10^{-8}$ as required. The interface with the screened-scalar literature [56] is established through this split, and the derived relation between the two sectors is the single open calculation that closes the screening story.

- **Open calculation.** Gate 10 (galactic coupling reconciliation) is closed: the pure potential-depth law with corrected $|\Phi|/c^2$ values and recalibrated $(\Phi_{\rm half}, n) = (1.59 \times 10^{-7}, 3.56)$ satisfies all three boundary conditions (absorber, Cassini, galactic) simultaneously. The single remaining open calculation is Gate 4B: the non-perturbative evaluation of $\mathcal{C}_{T,\parallel}$ must yield $\mathcal{A}_{\rm env} \approx 1.4 \times 10^4$. All perturbative mechanisms have been quantitatively excluded (disformal metric correction $\sim 10^{-128}$); the amplification must arise from a genuinely non-perturbative topological/geometric effect. If Gate 4B fails ($\mathcal{A}_{\rm env} < 10^3$), the framework is falsified.

![Environmental screening operator S_Σ vs |Φ|/c²](assets/gate4b_screening.png)

**Figure 3.1:** Environmental screening operator $\mathcal{S}_\Sigma(|\Phi|/c^2)$ (upper panel) and fifth-force strength $\beta_{\rm eff}^2 \times |\Phi|/c^2$ (lower panel) versus gravitational potential depth. This is the absorber-regime projection of the multi-variable operator $\mathcal{S}_\Sigma(\mathcal{E})$. The screening function is calibrated by three boundary conditions: the absorber regime ($|\Phi|/c^2 \sim 10^{-8}$, unscreened, $\mathcal{S}_\Sigma \approx 1$), the solar-system Cassini bound ($|\Phi|/c^2 = 2.12 \times 10^{-6}$, $\mathcal{S}_\Sigma \approx 10^{-4}$), and the galactic halo ($|\Phi|/c^2 = 5.38 \times 10^{-7}$, $\mathcal{S}_\Sigma \approx 0.013$). The $3.94\times$ potential-depth difference between solar system and galaxy yields $132\times$ screening contrast, sufficient to satisfy both Cassini and the galactic coupling requirement without a second screening variable (Gate 10 closed). The fifth-force strength peaks in the unscreened intermediate regime ($|\Phi|/c^2 \sim 10^{-7}$), confirming that the scalar effect is strongest in diffuse clouds and suppressed in dense environments.
...
The geometric and thermodynamic foundations of the TEP cosmological framework — the temporal horizon, curvature regularity ($0 < p \le 1/2$, all polynomial curvature invariants vanish), proper-time asymptotic regularity ($\Delta\tau \to \infty$), CMB blackbody spectral preservation, FIRAS Compton-exposure convergence ($a + p > 1$), and the full derivation of the chemical exposure convergence condition ($p + q > 1$) — are established in TEP-TH (Paper 27, [4]). The acoustic-sector closure, including $\Omega_b h^2$ determination from Planck peaks and sound-horizon preservation at $<6\text{ ppm}$, is established in TEP-HC (Paper 18, [3]) and TEP-C0 (Paper 26, [2]). The present paper does not re-derive these results; it applies the established framework to the BBN-specific questions of nuclear reaction flows, light-element equilibrium, and D/H spectroscopy.

The essential structural results inherited from TEP-TH are: (i) the effective scale factor $a_{\rm eff} = A_{\rm clock}$ with $A_{\rm clock}(z) = (1+z)^{-1}$; (ii) the redshift decomposition $\ln(1+z_T) = \int_\gamma (\Sigma_\parallel + \mathcal{C}_{T,\parallel})\,d\ell$; (iii) the temporal horizon limit $A_{\rm clock} \to 0$, $z \to \infty$, $\tau \to \infty$, $\mathcal{K} \to 0$; and (iv) the convergence conditions $p + q > 1$ (astration) and $a + p > 1$ (Compton exposure), both satisfied for the curvature-regular branch with $p = 0.5$, $q = 0.8$, giving $p + q = 1.3$.
...
- **Orphan population:** The velocity-selection theorem (Gate 6) predicts that $\sim 66\%$ of edge-shear features are absorbed into the H I component structure as orphan narrow components at non-isotope offsets. The quantitative forecast (Gate 6-II) predicts $\sim 16$ such features across $\sim 80$ high-S/N LLS sightlines. If a re-analysis of existing archival spectra shows no excess of narrow ($b < 10$\,km/s), low-column ($\log N \sim 12$--$14$) components at non-isotope offsets beyond the normal H I population, the TEP interpretation is substantially weakened.

- **Spectroscopic robustness:** If the free-H preference disappears under the physically motivated parent window ($\pm 3b_{\rm max}$) combined with limited scaffold freedom and a shared continuum basis across all sightlines, the spectroscopic evidence for a non-isotopic origin would be substantially weakened. The physical window robustness check (C1, Section 5.5) confirms the conclusions are unchanged under this tighter window.
...
The 43 H I components are held at the published literature architecture in the standard analysis. Allowing the five strongest non-parent H I components *within the absorber system* ($|v| \le 1000$ km/s, excluding the D parent components) to re-optimise under both the D and H hypotheses changes the likelihood gain from $T = 24.04$ (fixed scaffold) to $T = 73.86$ (five freed components, $\Delta T = +50$). Both models benefit from the additional freedom ($\Delta\ln L_D = +61.0$, $\Delta\ln L_H = +85.9$), but H benefits more, so the fixed-scaffold comparison is conservative. The D parent components are excluded from the freed set because the D model inherits the parent's Doppler parameter with mass scaling; freeing the parent's $b$ creates a conflicting optimization where D's $b$ is doubly controlled. A calibrated Monte Carlo with the same freed-scaffold optimization inside the simulation loop gives $p = 0.68$ (136/200 exceedances) — not significant. This result is a *limitation*, not a robustness confirmation: under the externally fixed published architecture, the free-H component is strongly preferred over canonical-parent D; when substantial nuisance freedom is introduced, the surrounding absorption model can absorb that discrimination. The fixed-scaffold $p$-values are the more discriminative numbers because freezing an independently published architecture is a legitimate test definition, but the freed-scaffold result shows that the evidence is conditional on the scaffold and does not survive when the absorber model is allowed to re-optimise. This is reported transparently as a limitation of the spectroscopic evidence.

#### Pure Kinematic Prior (C4)

To test whether the candidate feature is merely an ordinary H I component shifted by a free velocity, the data were also fitted with a two-parameter *kinematic* model: free $v_H$ and $\log N_H$, but $b$ fixed to the parent H I value (no isotope mass scaling and no free $b$). This model is strongly rejected: $\Delta\mathrm{AIC}(M_{\rm kin} - M_D) = +209.12$ and $\Delta\mathrm{AIC}(M_H - M_{\rm kin}) = -231.16$. The best-fit kinematic velocity is $v_H = -131.99$ km/s, but the parent-like $b$ value cannot reproduce the narrow isotopic core. A pure velocity shift without deuterium's mass-scaled Doppler parameter does not explain the feature.

#### Noisy/Offset True-D Monte Carlo (C3)

The standard calibration generates exact true-D flux from the maximum-likelihood D parameters. A more realistic test perturbs the generating true-D nuisance parameters before drawing each realisation: $\log N_D$ by $\pm 0.1$ dex, $T_K$ by $\pm 1000$ K (clamped to $> 100$ K), and $b_{\rm turb}$ by $\pm 1$ km s$^{-1}$. The D/H velocity offset ($-81.6$ km/s) is a fixed physical constant (exact reduced-mass isotope shift) and is not perturbed. For Q1009+2956 the result confirms the main analysis: 0/100 standard realisations exceed $T_{\rm obs}$ ($p_{\rm std} = 0.0099$), and 10/100 parent-reassignment realisations exceed $T_{\rm parent}$ ($p_{\rm parent} = 0.109$). The standard-statistic rejection is robust to realistic uncertainty in the true-D model parameters, and the parent-reassignment non-significance is likewise preserved. An earlier version of this check perturbed the velocity offset, creating a mismatch between the generating and fitting models that artificially inflated $T_{\rm sim}$; the corrected implementation confirms the original significance.

#### Physical Parent Window (C1) — Headline Statistic

The physically motivated parent window ($\pm 3b_{\rm max}$, where $b_{\rm max}$ is the largest Doppler parameter of the main complex) is adopted as the *headline* statistic, with the conservative $\pm 1000$\,km/s window reported as a lower bound. For Q1009+2956 the largest $b$-value of the main complex is 56.7 km s$^{-1}$, so the physical window $\pm 3b$ is $\pm 170$ km s$^{-1}$, compared with the conservative $\pm 1000$ km s$^{-1}$. This reduces eligible parents from 43 to 9. With 200 true-D Monte Carlo realisations the fair standard statistic gives 0/200 exceedances ($p_{\rm std} = 0.005$), while the parent-reassignment statistic gives 23/200 exceedances ($p_{\rm parent} = 0.119$). The result is essentially unchanged from the fixed window: the H alternative is preferred under the specific published parent assignment but is not significant when the parent is chosen adversarially. The $\pm 1000$ km s$^{-1}$ conservative window was therefore not driving the non-significance of the parent-reassignment test, and the physical window is adopted as the headline because it is motivated by the absorber's own velocity structure rather than an arbitrary fixed range.

### 5.6 Multi-Pronged Evidence Analysis (Gate 8)

Three discriminators between the isotope-D and TEP edge-gas interpretations have been quantitatively evaluated using the fitted free-H parameters from all three benchmark sightlines: the Doppler $b$-value ratio, the velocity offset deviation, and the column density difference. These quantities are all derived from the same free-H fit and are therefore not statistically independent; the joint multivariate statistic is the appropriate calibrated measure. A true-D injection Monte Carlo (step\_08b, $N = 30$ realisations per sightline) calibrates each statistic against the null distribution obtained by fitting $M_{H,\rm free}$ to simulated true-D spectra.

**b-value ratio.** Under isotope-D, the fitted free-H Doppler parameter should equal the mass-scaled parent value $b_D = b_{\rm parent}/\sqrt{2}$. Under TEP, $b$ is set by edge-gas temperature and is unconstrained. The observed ratios are $0.86$ (Q1009), $1.22$ (PKS\,1937), and $0.30$ (J1332). The J1332 ratio corresponds to pure thermal broadening at $T \sim 900$\,K, characteristic of cool edge gas, not of D co-located with the $T \sim 16\,000$\,K parent. After calibration against the true-D null, the individual $b$-value $p$-values are $1.00$ (Q1009), $0.97$ (PKS\,1937), and $0.60$ (J1332) — none are individually significant.

**Offset-deviation correlation.** Under isotope-D, all fitted free-H velocities should sit at the exact isotope shift within spectral resolution. Under TEP, the offset depends on the local shear amplitude, which scales with the absorber's gravitational potential. The observed deviations are $+1.1$\,km/s (Q1009), $+5.7$\,km/s (PKS\,1937), and $-20.5$\,km/s (J1332). The three sightlines exhibit the predicted monotonic ordering with $\log N_{\rm HI}$ (Spearman $\rho = 1.0$); however, with only three data points the sample is too small for the correlation itself to constitute significant evidence ($P(\rho = 1 \mid N = 3) = 0.17$ for a prespecified directional trend). The J1332 $-20.5$\,km/s deviation is the most interesting individually. After calibration, the individual deviation $p$-values are $1.00$, $0.97$, and $0.73$ — none individually significant.

**Sign test.** The conformal sign theorem predicts all edge-shear features should be blueward of the parent. All three are: $-132.9 < -52.4$, $-67.2 < +8.7$, $-119.5 < -17.4$\,km/s. Under the null, $P(3/3\ {\rm blueward}) = 0.125$.

**Column density difference.** Under isotope-D, $N_{H,\rm free}$ should match $N_{\rm HI} \times ({\rm D/H})_{\rm BBN}$. The observed differences are $-0.08$\,dex (Q1009), $-0.08$\,dex (PKS\,1937), and $+2.15$\,dex (J1332). After calibration, the individual $p$-values are $1.00$, $0.97$, and $0.90$ — none individually significant.

**Calibrated joint assessment.** The three statistics are correlated because they derive from the same free-H fit. A joint Mahalanobis distance against the true-D covariance matrix provides the calibrated multivariate test. Q1009$+$2956 and PKS\,1937$-$101 are both significant in the joint statistic ($p < 0.033$ for $N = 30$), indicating that the combination of fitted parameters is anomalous relative to what true-D simulations produce. J1332$+$0052 is not significant in the joint statistic ($p = 0.13$): the nominal $5$--$8\sigma$ values reported in an earlier analysis were inflated by comparing against spectral-resolution uncertainty rather than the true-D null distribution. The J1332 $b = 3.9$\,km/s and $-20.5$\,km/s deviation remain physically interesting — the low $b$ is consistent with cool edge gas — but they do not constitute a calibrated rejection of isotope-D at this sample size. The cross-sightline pattern — deviations scaling with $N_{\rm HI}$, all features blueward — is suggestive but not conclusive with $N = 3$. The parent-identity test is mixed ($1$--$1$ for meaningful sightlines), and the amplitude gap (Gate 4B) remains open. The cumulative case is strengthened but not conclusive; the population forecast (Gate 6-II) and orphan search protocol (Gate 6-III) provide the next testable predictions.

### 5.7 Orphan Search Protocol (Gate 6-III)

The population forecast (Gate 6-II) predicts $\sim 16$ orphan narrow components at non-isotope offsets across $\sim 80$ high-S/N LLS sightlines. A pilot search on $10$--$15$ sightlines from existing archival surveys would turn this prediction into a first observational test.

**Archival samples.** Three existing surveys provide suitable data without requiring new observations:

- *SQUAD* (Supersonic QSO Absorption-line Database, Zavarygin et al. 2018): $\sim 200$ UVES sightlines at $R \sim 40\,000$--$50\,000$, covering Ly$\alpha$--Ly$\gamma$ at $z > 2.5$. Approximately $30$--$40$ sightlines have LLS-class absorbers with sufficient S/N for narrow-component detection.
...
- **CMB acoustic peaks and sound horizon (TEP-HC, Paper 18).** The TEP conformal scalar field has been implemented natively in *hi_class* and run through MCMC against Planck 2018, BAO, and Pantheon+. The acoustic sound-horizon ratio $r_s^{\rm TEP}/r_s^{\Lambda{\rm CDM}} = 0.999994$ ($< 6$\,ppm deviation), and the TT/TE/EE peak morphology is preserved without early spatial expansion. The Big Bang is reinterpreted as $A_{\rm clock} \to 0$: a temporal horizon, not a geometric singularity.
...
The repository contains a deterministic, version-controlled cosmological analysis pipeline utilizing 13 analysis steps across 4 phases: core gates (1–8), disformal transport, secondary robustness analyses, and manuscript finalization.
...
All raw spectroscopic data (VLT/UVES and VLT/ESPRESSO), structural likelihood matrices, and the temporal-field equation solvers are released in the Zenodo repository (DOI: 10.5281/zenodo.21841148) under CC-BY 4.0. The full codebase and execution environments are identical to the published version.

#### Repository Structure

TEP-BBN/
├── data/
│   ├── raw/                       # Spectroscopic exposures (UVES, ESPRESSO)
...
├── scripts/
│   ├── steps/                     # 13 pipeline steps (01-09, 09b, secondary, finalize)
...
| J1332$+$0052 Spectra | VLT/UVES (Kislitsyn et al. 2024) | ESO Archive | 24 exposures | `data/raw/spectra/J1332+0052_z3.420_UVES/` |
...
| J1332$+$0052 Model | Kislitsyn et al. 2024 / VPFIT [50] | Static File | 4 H I + 4 D I components | `data/literature_components/J1332+0052_z3.420_vpfit_model.26` |
| Prior Bounds | Derived | Static File | All variables | `configs/sightlines/*.json` |

### Pipeline Architecture

The analysis pipeline comprises 4 phases and 13 steps spanning spectroscopic ingestion to thermodynamic evaluation, helium synthesis, opacity boundary analysis, disformal transport calibration, secondary robustness checks, and manuscript number finalization.
...
Three analysis fixes were applied between v0.1 and v0.2, each with a quantitative impact on the primary statistic:

| Fix | Description | Impact on $T$ (Q1009) |
| --- | --- | --- |
| Dynamic continuum basis | 10-parameter Chebyshev for Ly$\alpha$ regions (broad H I wing), 5-parameter for metal-line regions. v0.1 used a rigid 3-parameter basis that could not separate the H I wing from the D feature. | $T$: 56.54 → 24.04 |
...
| Per-sightline noise models | v0.1 used a single Q1009-calibrated noise model for all sightlines. v0.2 calibrates per-sightline Student-$t$ models from each absorber's D-model residuals. | PKS $T$: 8982 → 85.41; J1332 $T$: 1886 → 0.49 |
...
The net effect of these fixes is to *strengthen* the TEP thesis: the corrected identifiability ($0.10\sigma$ at actual D column) is far below the v0.1 claim ($5.25\sigma$), meaning H and D are substantially less distinguishable than previously reported. The primary statistic $T$ decreased from 56.54 to 24.04, but the Monte Carlo calibration yields $p_{\rm std} = 0.001$ (significant) and $p_{\rm parent} = 0.094$ (not significant) — a more honest and defensible result than the v0.1 headline.
