#!/usr/bin/env python3
"""
Step 60: Radial Discriminator (Gate F v2 - Phase F1)
=====================================================
Fits the CF4 TF catalog over all radial shells to test the phenomenological TEP limit
G_1(r) = r / r_* against the pure kinematic outflow G_K(r) = 1/r in log-distance space.

1. Bins CF4 TF by velocity.
2. Fits the kinematic bulk flow B_kin in each bin to test for V^2 growth (which confirms G_TEP = r).
3. Performs a global joint fit of MK, MT, and MKT to test the global significance.
"""
...
V_STAR = 10000.0  # ~100 Mpc/h for normalization of T
        ...
        for line in f:
            ...
            if dm_tf and pgc and vcmb and ra and dec:
                ...
                if vcmb_val < 2000 or vcmb_val > 20000:
                    continue  # standard cut to avoid local non-linearities and high-z noise
...
def log_like(params, model_type, y, v, nx, ny, nz, cos_theta, err):
    # y = 5 log10(V) - mu
    # M0: alpha
    # MK: alpha + (5/ln10) * (B dot n) / V
    # MT: alpha + T * (V / V_star) * cos_theta
    # MKT: alpha + T * (V / V_star) * cos_theta + (5/ln10) * (B dot n) / V
...
def fit_model(df, model_type):
    y = 5 * np.log10(df['vcmb'].values) - df['dm'].values
    v = df['vcmb'].values
    nx = df['nx'].values
    ny = df['ny'].values
    nz = df['nz'].values
...
def run_radial_discriminator():
    log = TEPLogger("step_60_radial", log_file_path=PROJECT_ROOT / "outputs/logs/step_60_radial.log")
    set_step_logger(log)
    log.info("Loading CF4 TF Catalog...")
    df = parse_cf4_tf()
    log.info(f"Loaded {len(df)} TF galaxies.")
    
    # 1. Global Model Comparison
    log.info("\n=== Global Model Comparison (All V) ===")
    ...
    log.info(f"MT (Temporal):       BIC = {bic_mt:.1f} | NLL = {res_mt.fun:.1f} | dBIC(to MK) = {bic_mt - bic_mk:.1f}")
    log.info(f"MKT (Kin+Temporal):  BIC = {bic_mkt:.1f} | NLL = {res_mkt.fun:.1f} | dBIC(to MT) = {bic_mkt - bic_mt:.1f}")
    
    T_global = res_mt.x[1]
    log.info(f"Global TEP Amplitude (MT): T = {T_global:.4f} mag at 100 Mpc/h")
    ...
    for vmin, vmax in bins:
        mask = (df['vcmb'] >= vmin) & (df['vcmb'] < vmax)
        df_bin = df[mask]
        if len(df_bin) < 50:
            continue
        ...
        v_center = np.median(df_bin['vcmb'])
        ...
        # What apparent flow does the Global MT model predict?
        # delta y_T = T_global * (V / V_STAR) * cos_theta
        # delta y_K = (5/ln10) * B_app / V * cos_theta
        # Equating them: B_app = T_global * (V^2 / V_STAR) * (ln10 / 5)
    ...
    plt.errorbar(bin_centers, b_parallel_fitted, yerr=np.ones_like(bin_centers)*50, fmt='o-', label='Fitted Kinematic B_|| (Data)', markersize=8)
    plt.plot(bin_centers, t_predicted, 'r--', label='TEP Transport Prediction (T * r^2)', linewidth=2)
    plt.axhline(0, color='k', linestyle=':')
    plt.xlabel("V_cmb (km/s)")
    plt.ylabel("Apparent CMB-Aligned Bulk Flow (km/s)")
    plt.title("Radial Growth of Apparent Flow: Kinematic vs Temporal")
    plt.legend()
